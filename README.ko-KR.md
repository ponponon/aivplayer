...
정식 Windows, Linux 및 macOS 설치 파일은 시작 후 백그라운드에서 GitHub Releases를 확인합니다. 새 버전이 발견되면 업데이트 대화상자에서 건너뛰기, 나중에 알림, 다운로드를 선택할 수 있습니다. 다운로드가 끝난 뒤 “재시작하여 업데이트”를 눌러야 종료 및 설치가 진행되며, 현재 재생이나 편집을 강제로 중단하지 않습니다. macOS에서는 앱 메뉴의 “업데이트 확인…”으로 수동 확인도 할 수 있습니다. 개발 모드, Windows Store 빌드와 `aivcli`는 자동 업데이트에 참여하지 않습니다.
...
이 문제는 이전 설치 파일이 LanceDB 런타임 의존성을 함께 패키징하지 않아 발생한 시작 오류입니다. 동영상 파일명, 외장 드라이브 경로 또는 MP4 인코딩이 원인이 아닙니다. 현재 `v0.6.1` 릴리스에는 수정 사항이 포함되어 있습니다. 해당 Release의 설치 파일을 다운로드하고 앱 번들 안에 npm 의존성을 직접 설치하지 마세요.

현재 릴리스는 `v0.6.1`이므로 해당 Release의 설치 파일을 우선 사용하세요.
