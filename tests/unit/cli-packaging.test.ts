...
describe('aivcli packaging integration', () => {
  it('ships platform launchers and installer hooks', () => {
    ...
    expect(builderConfig).toContain('productName: AIVPlayer')
    expect(readFileSync(join(projectRoot, 'build/deb-after-install.sh'), 'utf8')).toContain('APP_INSTALL_DIR="/opt/AIVPlayer"')
    expect(readFileSync(join(projectRoot, 'build/deb-after-remove.sh'), 'utf8')).toContain('APP_INSTALL_DIR="/opt/AIVPlayer"')
    expect(readFileSync(join(projectRoot, 'build/deb-after-install.sh'), 'utf8')).not.toContain('/opt/aivplayer')
    expect(readFileSync(join(projectRoot, 'build/deb-after-remove.sh'), 'utf8')).not.toContain('/opt/aivplayer')
