...
async function main(): Promise<void> {
  ...
  const contentHash = await createMediaContentHash(replacementPath)
  const project = {
    schemaVersion: 1,
    ...
    createdAt: 100,
    updatedAt: 100,
    sources: [{ id: 'source-repair-smoke', path: missingPath, name: 'old-location-name.mp4', fingerprint: `${missingPath}:${durationSeconds}`, contentHash, durationSeconds }],
  ...
  try {
    ...
    const stored = await page.evaluate(() => {
      const projects = JSON.parse(localStorage.getItem('aivplayer.editing-projects.v1') ?? '{}') as Record<string, { sources?: Array<{ id: string; path: string; contentHash?: string }>; captionSourcePaths?: Record<string, { source: string | null; translation: string | null }>; title?: string }>
      return Object.values(projects).find((candidate) => candidate.title === 'Repair summary Smoke') ?? null
    })
    const repairedSource = stored?.sources?.find((source) => source.id === 'source-repair-smoke') ?? null
    const contentHashBasisVerified = repairedSource?.contentHash === contentHash
    const result = { status: status ?? '', repairedSource, repairedCaptionPaths: stored?.captionSourcePaths?.['source-repair-smoke'] ?? null, contentHashBasisVerified, consoleErrors }
    console.log(`AIVPlayer Smoke Editing Project Repair\n${JSON.stringify(result)}`)
    if (!status?.includes('→') || !status.includes('固定') || repairedSource?.path !== replacementPath || repairedSource.id !== 'source-repair-smoke' || !contentHashBasisVerified || result.repairedCaptionPaths?.source !== null || consoleErrors.length > 0) process.exitCode = 1
  } finally {
    await app.close()
  }
}
