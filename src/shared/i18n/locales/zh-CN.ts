  export const zhCN = {
    ...
    editing: {
      subtitleQaTitle: '字幕 QA', subtitleQaIssueCount: (count: number) => count === 0 ? '通过' : `${count} 项`, subtitleQaClear: '未发现明显问题', subtitleQaJump: '定位', subtitleQaKindLabels: { empty: '空字幕', overlap: '时间重叠', 'too-short': '时长过短', 'too-long': '时长过长', 'high-cps': '阅读速度过快', 'wide-line': '行宽过长', punctuation: '标点异常', recognition: '疑似识别问题' }, subtitleQaMessage: (kind: string, value: number) => kind === 'overlap' ? `重叠 ${value.toFixed(2)} 秒` : kind === 'high-cps' ? `约 ${value.toFixed(1)} 字/秒` : kind === 'wide-line' ? `最长约 ${value.toFixed(1)} em` : kind === 'too-short' || kind === 'too-long' ? `时长 ${value.toFixed(2)} 秒` : '请检查字幕文本',
