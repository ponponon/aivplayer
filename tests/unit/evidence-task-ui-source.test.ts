...
describe('evidence task renderer contract', () => {
  it('keeps TTS drafts separate until an explicit formal subtitle import', () => {
    ...
    expect(drafts).toContain('mergeSelectedDrafts')
    expect(drafts).toContain('cues: selectedDrafts.flatMap')
    expect(list).toContain('vision-tts-confirm-import-button')
    expect(list).toContain('vision-tts-merge-drafts-button')
    expect(list).toContain('vision-tts-select-')
