...
describe('Linux application icon packaging', () => {
  it('uses a hicolor-compatible 512x512 source icon', () => {
    ...
    const desktopServicesSource = readFileSync(join(projectRoot, 'src/desktop/desktop-services.ts'), 'utf8')
    ...
    expect(builderSource).toContain('from: brand/icon-linux.png\n    to: app-icon-linux.png')
    expect(desktopServicesSource).toContain("process.platform === 'linux' ? 'icon-linux.png'")
    expect(desktopServicesSource).toContain("process.platform === 'linux' ? 'app-icon-linux.png'")
