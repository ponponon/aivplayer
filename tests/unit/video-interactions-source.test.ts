...
describe('video surface interaction source constraints', () => {
  it('separates single-click playback from double-click fullscreen', () => {
    ...
    expect(playerCss).toMatch(/\.stage:fullscreen\s*\{[^}]*display:\s*grid;[^}]*grid-template-rows:\s*minmax\(0,\s*1fr\)\s+auto;[^}]*grid-template-areas:\s*'media'\s+'controls';/s)
    expect(playerCss).toMatch(/\.stage:fullscreen\s+\.video-frame\s*\{[^}]*position:\s*relative;[^}]*inset:\s*auto;[^}]*grid-area:\s*media;/s)
    expect(playerCss).toMatch(/\.stage:fullscreen\s*>\s*\.subtitle-overlay\s*\{[^}]*position:\s*relative;[^}]*inset:\s*auto;[^}]*grid-area:\s*media;/s)
    expect(playerCss).toMatch(/\.stage:fullscreen\s+\.control-deck\s*\{[^}]*position:\s*relative;[^}]*grid-area:\s*controls;[^}]*margin:/s)
    expect(playerCss).toMatch(/\.stage:fullscreen\s+\.control-deck\.is-hidden\s*\{[^}]*max-height:\s*0;[^}]*padding:\s*0;[^}]*margin:\s*0;/s)
  })
})
