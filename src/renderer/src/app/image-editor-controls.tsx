...
export function ImageTransformToolbar({ copy, settings, onUpdate, onReset }: Pick<Props, 'copy' | 'settings' | 'onUpdate' | 'onReset'>): React.ReactElement {
  ...
  return <div className="image-transform-toolbar" aria-label={copy.quickAdjust}>
    ...
    <button className={`image-ghost-button ${settings.flipX ? 'active' : ''}`} type="button" onClick={() => onUpdate({ flipX: !settings.flipX })} title={copy.flipHorizontal} aria-label={copy.flipHorizontal} aria-pressed={settings.flipX}><FlipHorizontal size={15} /></button>
    <button className={`image-ghost-button ${settings.flipY ? 'active' : ''}`} type="button" onClick={() => onUpdate({ flipY: !settings.flipY })} title={copy.flipVertical} aria-label={copy.flipVertical} aria-pressed={settings.flipY}><FlipVertical size={15} /></button>
...
export function ImageEditorControls({ copy, image, settings, preview, isRendering, onUpdate, onReset, onExport, onExportLivePhoto, onRemove }: Props): React.ReactElement {
  ...
  return <aside className="image-inspector">
    ...
    <div className="image-control-section"><h3>{copy.outputSize}</h3><div className="image-dimension-grid"><label>{copy.width}<input className="image-number-input" type="number" min="1" value={settings.width} onChange={(event) => setDimension('width', event.target.value)} /></label><label>{copy.height}<input className="image-number-input" type="number" min="1" value={settings.height} onChange={(event) => setDimension('height', event.target.value)} /></label></div><button className={`image-lock-toggle ${settings.lockAspectRatio ? 'active' : ''}`} type="button" onClick={() => onUpdate({ lockAspectRatio: !settings.lockAspectRatio })} title={copy.keepRatio} aria-label={copy.keepRatio} aria-pressed={settings.lockAspectRatio}>{settings.lockAspectRatio ? <Unlock size={14} /> : <Lock size={14} />}{copy.keepRatio}</button></div>
