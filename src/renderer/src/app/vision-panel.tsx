...
import type { AppSelectChangeEvent } from '../../../shared/app-select'
...
const MAX_COLLECTION_REPAIR_DIRECTORY_CANDIDATES = 500
...
function formatCollectionRepairMatch(copy: LocaleCopy['vision'], match: VisionClipCollectionRepairMatch): string {
  ...
  if (match.basis === 'content-hash') return copy.collectionRepairBatchMatchedContentHash(match.missingFileName, replacement)
...
export function VisionPanel(): React.ReactElement {
  ...
  const [similarScan, setSimilarScan] = useState<VisionSimilarMediaScanResult | null>(null)
  ...
  const [similarThumbnailUrls, setSimilarThumbnailUrls] = useState<Record<string, string>>({})
  const [isSearching, setIsSearching] = useState(false)
  const [isCreatingProject, setIsCreatingProject] = useState(false)
  const [isSavingCollection, setIsSavingCollection] = useState(false)
  const [repairingCollectionId, setRepairingCollectionId] = useState<string | null>(null)
  const [isPreparingCollectionRepair, setIsPreparingCollectionRepair] = useState(false)
  const [isRepairingCollections, setIsRepairingCollections] = useState(false)
  const [collectionRepairPreview, setCollectionRepairPreview] = useState<CollectionRepairPreview | null>(null)
  ...
  const selectedCollectionsForRepair = selectedCollectionsForRename.filter((collection) => (collectionAvailability[collection.id]?.missingPaths ?? 0) > 0)
  ...
  const scanSimilarMedia = async (): Promise<void> => {
    if (isScanningSimilar) return
    setIsScanningSimilar(true)
    setError(null)
    try {
      setSimilarScan(await window.aiv.scanVisionSimilarMedia())
    } catch (reason) {
      if (reason instanceof Error && reason.name === 'AbortError') setError(app.copy.vision.librarySimilarCancelled)
      else setError(reason instanceof Error ? reason.message : String(reason))
    } finally {
  ...
  const cancelSimilarMedia = (): void => {
    if (!isScanningSimilar) return
    void window.aiv.cancelVisionSimilarMedia().catch((reason: unknown) => setError(reason instanceof Error ? reason.message : String(reason)))
  }
  ...
  useEffect(() => {
    let active = true
    const matches = similarScan?.groups.flatMap((group) => group.matches) ?? []
    void Promise.all(matches.map(async (match) => {
      if (!match.thumbnailPath) return [match.frameId, ''] as const
      try {
        return [match.frameId, await window.aiv.readVisionThumbnail(match.thumbnailPath)] as const
      } catch {
        return [match.frameId, ''] as const
      }
    })).then((entries) => {
      if (active) setSimilarThumbnailUrls(Object.fromEntries(entries))
    })
    return () => { active = false }
  }, [similarScan])
  ...
  const saveSelectedCollection = async (): Promise<void> => {
    const selectedResults = results.filter((result) => selectedResultIds.has(result.id))
    const selections = createVisionClipSelections(selectedResults)
    const title = collectionTitle.trim()
    if (!title || selections.length === 0 || isSavingCollection) return
    setIsSavingCollection(true)
    setError(null)
    try {
      const paths = [...new Set(selections.map((selection) => selection.videoPath))]
      const hashes = await Promise.all(paths.map(async (path) => [path, await window.aiv.getMediaContentHash(path).catch(() => null)] as const))
      const contentHashByPath = new Map(hashes)
      const selectionsWithHashes = selections.map((selection) => {
        const contentHash = contentHashByPath.get(selection.videoPath)
        return contentHash ? { ...selection, contentHash } : selection
      })
      const collection = await window.aiv.saveVisionClipCollection({ title, tags: normalizeVisionCollectionTags(collectionTags), sortMode: 'source-time', selections: selectionsWithHashes })
  ...
  const updateCollectionSelections = async (collection: VisionClipCollection, selections: VisionClipSelection[]): Promise<VisionClipCollection | null> => {
    if (isCollectionBatchBusy && repairingCollectionId === null) return null
  ...
  const updateCollectionFilterTags = (event: AppSelectChangeEvent, excluded: boolean): void => {
  ...
  const prepareSelectedCollectionRepair = async (source: 'files' | 'directory' = 'files'): Promise<void> => {
    if (isCollectionBatchBusy || selectedCollectionsForRepair.length === 0) return
    setIsPreparingCollectionRepair(true)
    setError(null)
    try {
      const paths = [...new Set(selectedCollectionsForRepair.flatMap((collection) => collection.selections.map((selection) => selection.videoPath)))]
      ...
      const replacements = source === 'directory'
        ? await (async () => {
          const directoryPath = await window.aiv.openFolderPicker({ title: app.copy.vision.collectionRepairBatchDirectoryAction })
          return directoryPath ? window.aiv.listMediaFilesInDirectory(directoryPath, true) : []
        })()
        : await window.aiv.openMediaFiles()
      if (replacements.length === 0) return
      if (source === 'directory' && replacements.length > MAX_COLLECTION_REPAIR_DIRECTORY_CANDIDATES) {
        setError(app.copy.vision.collectionRepairBatchDirectoryTooLarge(replacements.length))
        return
      }
      const replacementsWithMetadata = await mapWithConcurrency(replacements, 2, async (file) => {
        const [metadata, contentHash] = await Promise.all([
          window.aiv.getMediaMetadata(file.path).catch(() => null),
          window.aiv.getMediaContentHash(file.path).catch(() => null)
        ])
        return {
          ...file,
      ...
      setIsPreparingCollectionRepair(false)
    }
  }

  const applySelectedCollectionRepair = async (): Promise<void> => {
    if (!collectionRepairPreview || collectionRepairPreview.plan.ambiguousCount > 0 || collectionRepairPreview.plan.unmatchedCount > 0 || isRepairingCollections) return
    setIsRepairingCollections(true)
    setError(null)
    try {
      const replacementByPath = new Map(collectionRepairPreview.replacements.map((file) => [file.path, file]))
      const metadataPromises = new Map<string, Promise<MediaProbeMetadata | null>>()
      const getMetadata = (path: string): Promise<MediaProbeMetadata | null> => {
        const current = metadataPromises.get(path)
        if (current) return current
        const next = window.aiv.getMediaMetadata(path).catch(() => null)
        metadataPromises.set(path, next)
        return next
      }
      const updates = await Promise.all(collectionRepairPreview.collections.map(async (collection) => {
        const collectionMatches = collectionRepairPreview.plan.matches.filter((match) => match.collectionId === collection.id)
        const replacementByMissingPath = new Map(collectionMatches.map((match) => [match.missingPath, match.replacementPath]).filter((entry): entry is [string, string] => entry[1] !== null))
        const repairedSelections = await Promise.all(collection.selections.map(async (selection) => {
          const replacementPath = replacementByMissingPath.get(selection.videoPath)
          if (!replacementPath) return selection
          const replacement = replacementByPath.get(replacementPath)
          if (!replacement) throw new Error(app.copy.vision.collectionRepairNoMatch(selection.fileName))
          const metadata = await getMetadata(replacement.path)
          ...
          return {
            ...
            videoPath: replacement.path,
            fileName: replacement.name,
            fingerprint: replacement.fingerprint ?? `${replacement.path}:${durationSeconds}`,
            durationSeconds,
        ...
        return { collectionId: collection.id, selections: repairedSelections }
      }))
      const result = await window.aiv.updateVisionClipCollectionsSelections({ updates })
      if (!result.success) {
        setError(result.message)
        return
      }
      ...
      setCollections((current) => current.map((collection) => updatedById.get(collection.id) ?? collection))
      setSelectedCollectionIds(new Set())
      setCollectionRepairPreview(null)
      refreshCollectionOperation()
      setCollectionTransferStatus(result.message)
    } catch (reason: unknown) {
      setError(reason instanceof Error ? reason.message : String(reason))
    } finally {
      setIsRepairingCollections(false)
    }
  }

  const cancelSelectedCollectionRepair = (): void => {
    if (isRepairingCollections) return
    setCollectionRepairPreview(null)
  }
  ...
  const collectionRepairBatchPreview = collectionRepairPreview ? <div className="vision-card vision-collection-repair-preview" role="dialog" aria-label={app.copy.vision.collectionRepairBatchPreviewTitle}>
    ...
    <div className="vision-collection-repair-preview-actions"><button className="vision-primary-action" type="button" onClick={() => void applySelectedCollectionRepair()} disabled={isRepairingCollections || collectionRepairPreview.plan.ambiguousCount > 0 || collectionRepairPreview.plan.unmatchedCount > 0}><Check size={12} />{isRepairingCollections ? app.copy.vision.repairingCollectionSources : app.copy.vision.collectionRepairBatchApply}</button><button className="vision-secondary-action" type="button" onClick={cancelSelectedCollectionRepair} disabled={isRepairingCollections}><X size={12} />{app.copy.vision.collectionRepairBatchCancel}</button></div>
  </div> : null
  ...
  return <div className="vision-panel">
      ...
      {collectionImportPreviewItems.length > 0 ? <div className="vision-saved-search-import-conflicts vision-collection-import-preview-items" role="list" aria-label={app.copy.vision.collectionImportDecisionLabel}>
        {collectionImportPreviewItems.map((item) => {
          ...
          return <div className="vision-saved-search-import-conflict vision-collection-import-preview-item" key={item.incomingIndex} role="listitem" data-state={item.state}>
            <div><strong>{item.title}</strong><small>{stateLabel} · {app.copy.vision.collectionImportPreviewSelectionCount(item.selectionCount)}{item.missingSourceCount > 0 ? <em className="vision-collection-import-missing-source"> · {app.copy.vision.collectionImportPreviewMissingSource(item.missingSourceCount)}</em> : null}{localSummary}</small></div>
            {item.state === 'conflict' ? <label><span>{app.copy.vision.collectionImportDecisionLabel}</span><AppSelect value={collectionImportDecisions[String(item.incomingIndex)] ?? 'keep-local'} aria-label={`${app.copy.vision.collectionImportDecisionLabel}: ${item.title}`} onChange={(event) => setCollectionImportDecisions((current) => ({ ...current, [String(item.incomingIndex)]: event.target.value as VisionClipCollectionImportDecision }))} disabled={isTransferringCollectionImport}><option value="overwrite">{app.copy.vision.collectionImportOverwrite}</option><option value="keep-local">{app.copy.vision.collectionImportKeepLocal}</option><option value="skip">{app.copy.vision.collectionImportSkip}</option></AppSelect></label> : null}
          </div>
        })}
      </div> : null}
    ...
    <section className="vision-card vision-search-card">
      ...
      {selectedResultIds.size > 0 ? <div className="vision-selection-actions"><span>{app.copy.vision.selectedResults(selectedResultIds.size)}</span><input className="vision-collection-title-input" value={collectionTitle} onChange={(event) => setCollectionTitle(event.target.value)} placeholder={app.copy.vision.collectionTitlePlaceholder} aria-label={app.copy.vision.collectionTitlePlaceholder} /><input className="vision-collection-title-input" value={collectionTags} onChange={(event) => setCollectionTags(event.target.value)} placeholder={app.copy.vision.collectionTagsPlaceholder} aria-label={app.copy.vision.collectionTagsPlaceholder} /><button className="vision-secondary-action" type="button" onClick={() => void saveSelectedCollection()} disabled={!collectionTitle.trim() || isSavingCollection}><Archive size={14} />{isSavingCollection ? app.copy.vision.savingCollection : app.copy.vision.saveCollection}</button><button className="vision-primary-action" type="button" onClick={createProjectFromSelection} disabled={isCreatingProject}><FilePlus size={14} />{isCreatingProject ? app.copy.vision.creatingProject : app.copy.vision.createProject}</button></div> : null}
    </section>
    ...
    {collections.length > 0 ? <div className="vision-card vision-collection-tag-manager">
      ...
      {collectionTagImportPreview ? <div className="vision-collection-tag-manager-import-preview" role="dialog" aria-label={app.copy.vision.collectionTagManagerMetadataImportPreviewTitle}>
        ...
        {collectionTagImportConflicts.length > 0 ? <div className="vision-collection-tag-manager-import-conflicts" role="list" aria-label={app.copy.vision.collectionTagManagerMetadataImportDecisionLabel}>
          {collectionTagImportConflicts.map((item) => <div className="vision-collection-tag-manager-import-conflict" key={item.tag} role="listitem"><span>{item.tag}</span><label><span>{app.copy.vision.collectionTagManagerMetadataImportDecisionLabel}</span><AppSelect value={collectionTagImportDecisions[item.tag] ?? 'keep-local'} aria-label={`${app.copy.vision.collectionTagManagerMetadataImportDecisionLabel}: ${item.tag}`} onChange={(event) => setCollectionTagImportDecisions((current) => ({ ...current, [item.tag]: event.target.value as VisionClipCollectionTagMetadataImportDecision }))} disabled={isTransferringCollectionTagMetadata}><option value="overwrite">{app.copy.vision.collectionTagManagerMetadataImportOverwrite}</option><option value="keep-local">{app.copy.vision.collectionTagManagerMetadataImportKeepLocal}</option><option value="skip">{app.copy.vision.collectionTagManagerMetadataImportSkip}</option></AppSelect></label></div>)}
      ...
      <details className="vision-collection-tag-history" open>
        ...
        <div className="vision-collection-tag-history-toolbar">
          <label><span>{app.copy.vision.collectionTagManagerHistoryFilterLabel}</span><AppSelect value={collectionTagHistoryFilter} onChange={(event) => changeCollectionTagHistoryFilter(event.target.value as VisionClipCollectionTagOperationHistoryFilter)} aria-label={app.copy.vision.collectionTagManagerHistoryFilterLabel} disabled={isCollectionBatchBusy || isCollectionTagHistoryBusy}>
      ...
      {collectionTagStats.length > 0 ? <>
        ...
        <div className="vision-collection-tag-manager-metadata">
          ...
          <div className="vision-collection-tag-manager-metadata-controls">
            <label><span>{app.copy.vision.collectionTagManagerMetadataParentLabel}</span><AppSelect value={collectionTagParent} onChange={(event) => setCollectionTagParent(event.target.value)} aria-label={app.copy.vision.collectionTagManagerMetadataParentLabel} disabled={isCollectionBatchBusy}><option value="">{app.copy.vision.collectionTagManagerMetadataParentNone}</option>{collectionTagParentOptions.map((item) => <option key={item.tag} value={item.tag}>{item.tag}</option>)}</AppSelect></label>
        ...
        {savedCollectionFilterImportPreview ? <div className="vision-saved-search-import-preview" role="dialog" aria-label={app.copy.vision.collectionFilterSavedViewsImportPreviewTitle}>
          ...
          {savedCollectionFilterImportConflicts.length > 0 ? <div className="vision-saved-search-import-conflicts" role="list">
            {savedCollectionFilterImportConflicts.map((item) => <div className="vision-saved-search-import-conflict" key={item.incoming.id} role="listitem">
              ...
              <label><span>{app.copy.vision.collectionFilterSavedViewsImportDecisionLabel}</span><AppSelect value={savedCollectionFilterImportDecisions[item.incoming.id] ?? 'keep-local'} aria-label={`${app.copy.vision.collectionFilterSavedViewsImportDecisionLabel}: ${item.incoming.name}`} onChange={(event) => setSavedCollectionFilterImportDecisions((current) => ({ ...current, [item.incoming.id]: event.target.value as VisionClipCollectionSavedFilterImportDecision }))} disabled={isCollectionBatchBusy}><option value="overwrite">{app.copy.vision.collectionFilterSavedViewsImportOverwrite}</option><option value="keep-local">{app.copy.vision.collectionFilterSavedViewsImportKeepLocal}</option><option value="skip">{app.copy.vision.collectionFilterSavedViewsImportSkip}</option></AppSelect></label>
            </div>)}
    ...
    {collectionOperationHistory.length > 0 ? <div className="vision-card vision-collection-operation-history">
      ...
      <div className="vision-collection-operation-history-toolbar">
        <label><span>{app.copy.vision.collectionOperationHistoryFilterTypeLabel}</span><AppSelect value={collectionOperationHistoryTypeFilter} onChange={(event) => setCollectionOperationHistoryTypeFilter(event.target.value as VisionClipCollectionOperationHistoryTypeFilter)} aria-label={app.copy.vision.collectionOperationHistoryFilterTypeLabel} disabled={isCollectionBatchBusy}>
