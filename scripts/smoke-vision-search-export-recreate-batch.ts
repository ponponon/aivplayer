...
const failedTaskIds = ['smoke-failed-export-a', 'smoke-failed-export-b'] as const
  ...
  try {
    ...
    await writeFile(taskManifestPath, JSON.stringify({
      ...
      tasks: failedTaskIds.map((taskId, index) => ({
        ...
        outputPath: join(userDataDirectory, `smoke-results-${index}.json`),
    ...
    const manifest = await waitForFileState<ExportManifest>(taskManifestPath, (value) => value.tasks.length >= 4 && value.tasks.filter((task) => !failedTaskIds.includes(task.taskId as typeof failedTaskIds[number]) && task.searchRevision !== undefined).length >= 2)
    const recreated = manifest.tasks.filter((task) => !failedTaskIds.includes(task.taskId as typeof failedTaskIds[number]))
    if (recreated.length < 2) throw new Error(`Batch recreated tasks were not persisted: ${JSON.stringify(manifest)}`)
    ...
    console.log(`AIVPlayer Smoke Vision Search Export Recreate Batch passed: ${JSON.stringify({ originalTaskIds: failedTaskIds, recreatedTaskIds: recreated.slice(0, 2).map((task) => task.taskId), sharedRevision: JSON.stringify(recreated[0]?.searchRevision) === JSON.stringify(recreated[1]?.searchRevision) })}`)
  } finally {
