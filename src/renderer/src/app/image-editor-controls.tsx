...
export function ImageEditorControls({ copy, image, settings, preview, isRendering, onUpdate, onReset, onExport, onExportLivePhoto, onRemove }: Props): React.ReactElement {
  ...
  return <aside className="image-inspector">
    ...
    <div className="image-control-section"><label className="image-field-label" htmlFor="image-format">{copy.format}</label><select id="image-format" className="image-select" value={settings.format} onChange={(event) => onUpdate({ format: event.target.value as ImageSettings['format'] })}><option value="original">{copy.formatOriginal}</option><option value="jpeg">{copy.formatJpeg}</option><option value="webp">{copy.formatWebp}</option><option value="png">{copy.formatPng}</option></select><div className="image-quality-row"><label htmlFor="image-quality">{copy.quality}</label><output>{Math.round(settings.quality * 100)}%</output></div><input id="image-quality" className="image-range" type="range" min="0.1" max="1" step="0.01" value={settings.quality} disabled={settings.useTargetSize} onChange={(event) => onUpdate({ quality: Number(event.target.value) })} /></div>
    <div className={`image-control-section image-target-section ${settings.useTargetSize ? 'active' : ''}`}><label className="image-check-row"><input type="checkbox" checked={settings.useTargetSize} onChange={(event) => onUpdate({ useTargetSize: event.target.checked })} /><span><strong>{copy.targetSize}</strong><small>{copy.targetSizeHint}</small></span></label>{settings.useTargetSize ? <div className="image-target-input"><input className="image-number-input" type="number" min="1" step="0.1" aria-label={copy.targetSizePlaceholder} value={Number(targetValue.toFixed(2))} onChange={(event) => setTargetSize(event.target.value)} /><select className="image-unit-select" value={targetUnit} onChange={(event) => { const nextUnit = event.target.value as 'KB' | 'MB'; setTargetUnit(nextUnit); onUpdate({ targetSizeBytes: Math.max(1024, Math.round(targetValue * (nextUnit === 'MB' ? 1024 * 1024 : 1024))) }) }}><option value="KB">{copy.kb}</option><option value="MB">{copy.mb}</option></select></div> : null}{settings.useTargetSize && settings.format === 'png' ? <small className="image-control-hint">{copy.targetPngHint}</small> : null}</div>
