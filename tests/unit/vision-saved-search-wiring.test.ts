...
describe('vision saved search wiring', () => {
  it('keeps the saved search IPC surface connected end to end', () => {
    ...
    expect(channels).toContain("VISION_SAVED_SEARCH_EXPORT: 'vision:saved-search-export'")
    expect(channels).toContain("VISION_SAVED_SEARCH_IMPORT: 'vision:saved-search-import'")
    ...
    expect(ipc).toContain('VISION_SAVED_SEARCH_EXPORT')
    expect(ipc).toContain('VISION_SAVED_SEARCH_IMPORT')
    ...
    expect(preload).toContain('exportVisionSavedSearches')
    expect(preload).toContain('importVisionSavedSearches')
  })
})
