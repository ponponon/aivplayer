...
type TagOperationBatchDirection = 'undo' | 'redo'

type TagOperationBatchState = {
  collections: Map<string, TagOperationCollectionSnapshot>
  metadata: Map<string, VisionClipCollectionTagMetadata>
}

type TagOperationBatchTransition = {
...
function normalizeTagOperationIds(value: unknown): string[] {
  ...
  return [...new Set(value.filter((item): item is string => typeof item === 'string').map((item) => item.trim()).filter(Boolean))].slice(0, TAG_OPERATION_HISTORY_RETENTION_LIMIT)
}
...
export class ClipInboxStore {
  ...
  updateCollectionFlags(request: VisionClipCollectionFlagUpdateRequest): Pick<import('../../shared/vision-types').VisionClipCollectionFlagUpdateResult, 'collections' | 'skippedCount'> {
    ...
    const changed = collectionFlags.some((item) => (hasFavoritePatch && item.isFavorite !== request.isFavorite) || (hasArchivedPatch && item.isArchived !== request.isArchived))
    if (!changed) {
    ...
    const now = Date.now()
    snapshot.redoCollectionFlags = collectionFlags.map((item) => ({
      ...item,
  ...
  undoCollectionOperation(operationId?: unknown): VisionClipCollectionOperationUndoResult {
    ...
    if (operation.type === 'merge') {
      ...
      const currentCollections = createdCollections.map((collection) => this.getCollection(collection.id))
      if (currentCollections.some((collection, index) => !collection || JSON.stringify(collection) !== JSON.stringify(createdCollections[index]))) {
        return { success: false, message: '合并结果已被修改或删除，无法安全撤销', operation: null, collections: [] }
      }
      const now = Date.now()
      this.database.exec('BEGIN')
      try {
        ...
        this.database.exec('COMMIT')
        return { success: true, message: '已撤销上次合并操作', operation, collections: [], deletedCollectionIds: createdCollections.map((collection) => collection.id) }
      } catch (error) {
        this.database.exec('ROLLBACK')
        throw error
      }
    ...
    if (operation.type === 'delete') {
      ...
      if (removedCollections.some((collection) => this.getCollection(collection.id) !== null)) {
        return { success: false, message: '删除集合 ID 已被占用，无法安全恢复', operation: null, collections: [] }
      }
      const now = Date.now()
      this.database.exec('BEGIN')
      try {
        for (const collection of removedCollections) this.insertCollectionSnapshot(collection)
        this.database.prepare('UPDATE clip_collection_operation_history SET undone_at = ?, redoable = 1 WHERE id = ?').run(now, operation.id)
        this.database.exec('COMMIT')
        const collections = removedCollections.map((item) => this.getCollection(item.id)).filter((collection): collection is VisionClipCollection => collection !== null)
        return { success: true, message: '已撤销上次删除操作', operation, collections, createdCollectionIds: removedCollections.map((collection) => collection.id) }
      } catch (error) {
        this.database.exec('ROLLBACK')
        throw error
      }
  ...
  redoCollectionOperation(operationId?: unknown): VisionClipCollectionOperationRedoResult {
    ...
    if (operation.type === 'merge') {
      const createdCollections = snapshot.createdCollections
      if (!createdCollections || createdCollections.length === 0) return { success: false, message: '合并操作没有可重做快照', operation: null, collections: [] }
      if (createdCollections.some((collection) => this.getCollection(collection.id) !== null)) {
        return { success: false, message: '合并结果已存在，无法安全重做', operation: null, collections: [] }
      }
      this.database.exec('BEGIN')
      try {
        for (const collection of createdCollections) this.insertCollectionSnapshot(collection)
    ...
    if (operation.type === 'delete') {
      const removedCollections = snapshot.removedCollections
      if (!removedCollections || removedCollections.length === 0) return { success: false, message: '删除操作没有可重做快照', operation: null, collections: [] }
      const currentCollections = removedCollections.map((collection) => this.getCollection(collection.id))
      if (currentCollections.some((collection, index) => !collection || JSON.stringify(collection) !== JSON.stringify(removedCollections[index]))) {
        return { success: false, message: '恢复后的集合已被修改或删除，无法安全重做', operation: null, collections: [] }
      }
      this.database.exec('BEGIN')
      try {
        const placeholders = removedCollections.map(() => '?').join(', ')
        this.database.prepare(`DELETE FROM clip_collections WHERE id IN (${placeholders})`).run(...removedCollections.map((collection) => collection.id))
        this.database.prepare('UPDATE clip_collection_operation_history SET undone_at = NULL, redoable = 0 WHERE id = ?').run(operation.id)
        this.database.exec('COMMIT')
        return { success: true, message: '已重做上次删除操作', operation, collections: [], deletedCollectionIds: removedCollections.map((collection) => collection.id) }
      } catch (error) {
        this.database.exec('ROLLBACK')
        throw error
      }
  ...
  undoTagOperations(operationIds: readonly unknown[]): VisionClipCollectionTagOperationBatchUndoResult {
    return this.applyTagOperationBatch('undo', operationIds)
  }

  redoTagOperations(operationIds: readonly unknown[]): VisionClipCollectionTagOperationBatchRedoResult {
    return this.applyTagOperationBatch('redo', operationIds)
  }
  ...
  mergeCollections(collectionIds: readonly string[], title: unknown, sortMode: unknown = 'source-time', selectedSelections?: readonly VisionClipCollectionMergeSelection[]): { collection: VisionClipCollection; sourceIds: string[]; skippedCount: number } {
    ...
    this.recordCollectionOperation('merge', { createdCollections: [collection] }, collection.updatedAt)
  ...
  deleteCollections(collectionIds: readonly string[]): VisionClipCollectionBatchDeleteResult {
    ...
    try {
      ...
      if (deletedIds.length > 0) {
        const removedCollections = deletedIds.map((collectionId) => this.getCollection(collectionId)).filter((collection): collection is VisionClipCollection => collection !== null)
        ...
        this.recordCollectionOperation('delete', { removedCollections }, Date.now())
      }
      this.database.exec('COMMIT')
  ...
  private applyTagOperationBatch(direction: TagOperationBatchDirection, operationIds: readonly unknown[]): VisionClipCollectionTagOperationBatchUndoResult {
    ...
    const state: TagOperationBatchState = {
      collections: new Map(this.listCollections().map((collection) => [collection.id, { id: collection.id, tags: [...collection.tags], updatedAt: collection.updatedAt }])),
    ...
    this.database.exec('BEGIN')
    try {
      for (const transition of transitions) this.restoreTagOperationSnapshot(transition.snapshot, direction === 'redo')
      const updateHistory = direction === 'undo'
        ? this.database.prepare('UPDATE clip_tag_operation_history SET undone_at = ?, redoable = 1 WHERE id = ?')
        : this.database.prepare('UPDATE clip_tag_operation_history SET undone_at = NULL, redoable = 0 WHERE id = ?')
  ...
  private prepareTagOperationBatchTransition(operation: VisionClipCollectionTagOperationHistory, snapshot: TagOperationSnapshot, direction: TagOperationBatchDirection, state: TagOperationBatchState): TagOperationBatchPreparation {
    ...
    const afterCollections = snapshot.redoCollectionTags
    const afterMetadata = snapshot.redoMetadata
    if (!afterCollections || !snapshot.redoMetadataTags || !afterMetadata) return { transition: null, message: corruptedMessage }
    if (beforeCollections.length !== afterCollections.length) return { transition: null, message: corruptedMessage }

    const expectedCollections = direction === 'undo' ? afterCollections : beforeCollections
    const nextCollections = direction === 'undo' ? beforeCollections : afterCollections
    const seenCollectionIds = new Set<string>()
    for (let index = 0; index < expectedCollections.length; index += 1) {
      const expected = expectedCollections[index]
      const next = nextCollections[index]
    ...
    const beforeMetadata = this.materializeTagOperationBeforeMetadata(snapshot)
    if (!beforeMetadata) return { transition: null, message: corruptedMessage }
    const expectedMetadata = direction === 'undo' ? afterMetadata : beforeMetadata
    ...
    return {
      transition: {
        operation,
        snapshot,
        touchedCollectionIds: [...seenCollectionIds]
      },
      message: ''
    }
  }

  private materializeTagOperationBeforeMetadata(snapshot: TagOperationSnapshot): VisionClipCollectionTagMetadata[] | null {
    if (!snapshot.redoMetadata || !snapshot.redoMetadataTags) return null
    const metadata = this.toTagMetadataMap(snapshot.redoMetadata)
    if (!metadata) return null
    for (const tag of snapshot.metadataTags) metadata.delete(tag)
    for (const item of snapshot.metadata) metadata.set(item.tag, { ...item })
    return [...metadata.values()]
  }

  private toTagMetadataMap(metadata: readonly VisionClipCollectionTagMetadata[]): Map<string, VisionClipCollectionTagMetadata> | null {
    const result = new Map<string, VisionClipCollectionTagMetadata>()
    for (const item of metadata) {
      if (result.has(item.tag)) return null
      result.set(item.tag, { ...item })
    }
    return result
  }

  private tagMetadataMapsMatch(actual: ReadonlyMap<string, VisionClipCollectionTagMetadata>, expected: ReadonlyMap<string, VisionClipCollectionTagMetadata>): boolean {
    if (actual.size !== expected.size) return false
    for (const [tag, expectedMetadata] of expected) {
      const actualMetadata = actual.get(tag)
      if (!actualMetadata || JSON.stringify(actualMetadata) !== JSON.stringify(expectedMetadata)) return false
    }
    return true
  }
  ...
  private parseCollectionOperationSnapshot(value: unknown): CollectionOperationSnapshot | null {
    ...
    try {
      ...
      const readCollections = (items: unknown[]): VisionClipCollection[] => items.map((item) => {
        if (!item || typeof item !== 'object') return null
        const candidate = item as Partial<VisionClipCollection>
        if (typeof candidate.id !== 'string' || !candidate.id.trim() || typeof candidate.title !== 'string' || !candidate.title.trim() || !Array.isArray(candidate.tags) || !Array.isArray(candidate.selections) || typeof candidate.isFavorite !== 'boolean' || typeof candidate.isArchived !== 'boolean' || typeof candidate.createdAt !== 'number' || !Number.isFinite(candidate.createdAt) || typeof candidate.updatedAt !== 'number' || !Number.isFinite(candidate.updatedAt)) return null
        const selections = candidate.selections.map(normalizeSelection).filter((selection): selection is VisionClipSelection => selection !== null)
        if (selections.length === 0) return null
        return {
          id: candidate.id.trim(),
          title: candidate.title.trim(),
          tags: normalizeVisionCollectionTags(candidate.tags),
          sortMode: normalizeVisionCollectionSortMode(candidate.sortMode),
      ...
      }).filter((item): item is VisionClipCollection => item !== null)
      const collectionFlags = parsed.collectionFlags === undefined ? undefined : readFlags(parsed.collectionFlags)
