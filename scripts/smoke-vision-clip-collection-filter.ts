import { readAppSelectValue, readAppSelectValues, selectAppOption } from './smoke-select.ts'
...
async function runSmoke(): Promise<void> {
  ...
  try {
    ...
    await selectAppOption(page, collectionListSort, 'title-asc')
    ...
    await selectAppOption(page, collectionListSort, 'selection-count-desc')
    ...
    await selectAppOption(page, collectionListSort, 'duration-desc')
    ...
    await selectAppOption(page, visibilitySelect, 'favorites')
    await page.getByRole('status').filter({ hasText: '显示 1 / 3 个集合' }).waitFor({ timeout: 10_000 })
    ...
    await selectAppOption(page, visibilitySelect, 'all')
    ...
    await page.getByRole('button', { name: '集合状态: 全部 3', exact: true }).click()
    await selectAppOption(page, visibilitySelect, 'archived')
    await page.getByRole('status').filter({ hasText: '显示 1 / 3 个集合' }).waitFor({ timeout: 10_000 })
    ...
    await selectAppOption(page, visibilitySelect, 'active')
    await page.getByRole('status').filter({ hasText: '显示 2 / 3 个集合' }).waitFor({ timeout: 10_000 })
    ...
    await selectAppOption(page, tagSelect, { label: '采访' })
    await page.getByRole('status').filter({ hasText: '显示 2 / 3 个集合' }).waitFor({ timeout: 10_000 })
    ...
    await selectAppOption(page, tagSelect, [{ label: '采访' }, { label: '精选' }])
    await page.getByRole('status').filter({ hasText: '显示 2 / 3 个集合' }).waitFor({ timeout: 10_000 })
    ...
    await selectAppOption(page, tagMode, 'all')
    await page.getByRole('status').filter({ hasText: '显示 1 / 3 个集合' }).waitFor({ timeout: 10_000 })
    ...
    await selectAppOption(page, excludedTagSelect, { label: '采访' })
    await page.getByRole('status').filter({ hasText: '显示 1 / 3 个集合' }).waitFor({ timeout: 10_000 })
    ...
    await selectAppOption(page, persistedTagMode, 'all')
    await selectAppOption(page, excludedTagSelect, { label: '室内' })
    await selectAppOption(page, visibilitySelect, 'favorites')
    if ((await readAppSelectValue(collectionListSort)) !== 'title-asc') throw new Error('Collection list sort should remain independently selectable from filter conditions')
    await page.getByRole('status').filter({ hasText: '显示 1 / 3 个集合' }).waitFor({ timeout: 10_000 })
    ...
    await page.getByRole('status').filter({ hasText: '显示 1 / 3 个集合' }).waitFor({ timeout: 10_000 })
    const restoredTags = await readAppSelectValues(restoredTagSelect)
    const restoredExcludedTags = await readAppSelectValues(restoredExcludedTagSelect)
    const restoredQuery = await restoredQueryInput.inputValue()
    const restoredVisibility = await readAppSelectValue(restoredVisibilitySelect)
    const restoredMode = await readAppSelectValue(restoredTagMode)
    const restoredCollectionListSortMode = await readAppSelectValue(restoredCollectionListSort)
    ...
    await page.getByRole('status').filter({ hasText: '显示 1 / 3 个集合' }).waitFor({ timeout: 10_000 })
    if (await page.locator('.vision-collection').count() !== 1 || await page.getByRole('button', { name: '移除标签筛选: 采访', exact: true }).count() !== 1 || await page.getByRole('button', { name: '移除标签筛选: 精选', exact: true }).count() !== 1 || await readAppSelectValue(visibilitySelect) !== 'favorites') {
    ...
    await page.getByRole('dialog', { name: '导入筛选视图预览', exact: true }).waitFor({ timeout: 10_000 })
    const conflictDecision = page.locator('.vision-saved-search-import-conflict .app-select')
