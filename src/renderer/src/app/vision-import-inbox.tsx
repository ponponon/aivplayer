...
export function VisionImportInbox({ copy, directories, items, progress, pipelineProgress, isBusy, error, writeSidecars, onAddFolder, onRemoveFolder, onScan, onQueue, onIgnore, onRetry, onBatchQueue, onBatchIgnore, onBatchRetry, onBatchClear, onWriteSidecarsChange, onUpdateMetadata }: VisionImportInboxProps): React.ReactElement {
  ...
  return <section className="vision-import-inbox">
    ...
    <div className="vision-inbox-filter-row"><input className="vision-inbox-meta-input" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={copy.inboxSearchPlaceholder} aria-label={copy.inboxSearchPlaceholder} /><AppSelect className="vision-inbox-filter-select" value={status} onChange={(event) => setStatus(event.target.value as MediaImportInboxStatus | 'all')} aria-label={copy.inboxFilterAll}><option value="all">{copy.inboxFilterAll}</option><option value="discovered">{copy.inboxStatus('discovered')}</option><option value="queued">{copy.inboxStatus('queued')}</option><option value="processing">{copy.inboxStatus('processing')}</option><option value="ready">{copy.inboxStatus('ready')}</option><option value="failed">{copy.inboxStatus('failed')}</option><option value="ignored">{copy.inboxStatus('ignored')}</option><option value="missing">{copy.inboxStatus('missing')}</option></AppSelect><label className="vision-folder-option"><input type="checkbox" checked={favoriteOnly} onChange={(event) => setFavoriteOnly(event.target.checked)} /><span>{copy.inboxFavorite}</span></label></div>
