...
describe('vision clip collection operation wiring', () => {
  it('keeps collection updates and undo/redo behind shared IPC, desktop and preload', () => {
    ...
    const types = readFileSync(join(projectRoot, 'src/shared/vision-types.ts'), 'utf8')
    ...
    expect(desktop).toContain('operations: [], collections: [], conflicts: []')
    ...
    expect(store).toContain('VisionClipCollectionOperationBatchConflict')
    expect(types).toContain('VisionClipCollectionOperationBatchConflictReason')
    expect(types).toContain('conflicts: VisionClipCollectionOperationBatchConflict[]')
  })
})
