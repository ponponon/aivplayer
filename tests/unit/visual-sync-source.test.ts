...
describe('visual sync integration', () => {
  it('keeps the selected-caption sync control on the existing undoable caption actions', () => {
    ...
    expect(control).toContain('data-testid="editing-caption-alignment-preview"')
    expect(control).toContain('proposeEditingCaptionAlignment')
    expect(control).toContain('data-testid="editing-caption-alignment-apply"')
