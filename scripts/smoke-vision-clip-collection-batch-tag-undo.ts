...
async function runSmoke(): Promise<void> {
  ...
  try {
    ...
    await page.getByRole('status').filter({ hasText: '已撤销上次标签操作' }).waitFor({ timeout: 10_000 })
    persisted = await page.evaluate(() => window.aiv.listVisionClipCollections())
    if (JSON.stringify(persisted.find((collection) => collection.id === originals[0]?.id)?.tags) !== JSON.stringify(['海边', '采访']) || JSON.stringify(persisted.find((collection) => collection.id === originals[1]?.id)?.tags) !== JSON.stringify(['海边']) || JSON.stringify(persisted.find((collection) => collection.id === originals[2]?.id)?.tags) !== JSON.stringify(['室内'])) throw new Error(`Empty batch undo should restore all tags: ${JSON.stringify(persisted)}`)
    if (await page.getByRole('button', { name: '撤销上次标签操作', exact: true }).count() !== 0) throw new Error('Batch tag history should be exhausted after both undos')
    ...
    console.log(`AIVPlayer Smoke Vision Clip Collection Batch Tag Undo passed: ${JSON.stringify({ originalCount: originals.length, appendedUndo: true, emptyBatchUndo: true, untouchedPreserved: true, historyExhausted: true, screenshotPath: screenshotPath ?? null })}`)
  } finally {
