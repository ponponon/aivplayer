...
function normalizeContentHash(value: string | undefined): string | undefined {
  return normalizeMediaContentHash(value)
}
...
export function matchEditingSourceRepairCandidates(sources: readonly EditingSource[], candidates: readonly EditingSourceRepairCandidate[]): EditingSourceRepairMatch {
  ...
  for (const source of sources) {
    ...
    const sourceContentHash = normalizeContentHash(source.contentHash)
    const contentHashMatches = sourceContentHash
      ? available.filter((candidate) => normalizeContentHash(candidate.contentHash) === sourceContentHash)
      : []
    ...
    const matches = contentHashMatches.length === 1
      ? contentHashMatches
      : contentHashMatches.length > 1
        ? contentHashMatches
        : exact.length === 1
      ? exact
      : exact.length > 1
    ...
    if (matches.length !== 1) {
      if (matches.length > 1 || exact.length > 1 || sameName.length > 1 || sameDuration.length > 1) {
        const ambiguousCandidates = contentHashMatches.length > 1 ? contentHashMatches : matches.length > 1 ? matches : exact.length > 1 ? exact : sameName.length > 1 ? sameName : sameDuration
...
export function relinkEditingProjectSources(project: EditingProject, replacements: readonly EditingSourceRepairReplacement[], updatedAt = Date.now()): EditingProject | null {
  ...
  const sources = project.sources.map((source) => {
    ...
    const reboundSource: EditingSource = {
      ...source,
    ...
    if (replacement.contentHash) reboundSource.contentHash = normalizeContentHash(replacement.contentHash)
    else delete reboundSource.contentHash
