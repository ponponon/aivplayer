...
import { PlaybackComparison } from './playback-comparison'
...
import { getEditingCaptionLayout, getEditingCaptionLineLayout } from '../../../core/editing/caption-layout'
...
export function PlayerStage(): React.ReactElement {
  ...
  const hasEditingSourceCaption = app.isEditingMode && (app.editingProject?.captions.some((caption) => caption.kind === 'source' && app.editingCurrentTime >= caption.startSeconds && app.editingCurrentTime < caption.startSeconds + caption.durationSeconds) ?? false)
  const hasEditingTranslationCaption = app.isEditingMode && (app.editingProject?.captions.some((caption) => caption.kind === 'translation' && app.editingCurrentTime >= caption.startSeconds && app.editingCurrentTime < caption.startSeconds + caption.durationSeconds) ?? false)
  const showEditingSourceCaptionLayout = hasEditingSourceCaption && (app.appSettings.subtitles.displayMode !== 'translation' || !hasEditingTranslationCaption)
  const showEditingTranslationCaptionLayout = hasEditingTranslationCaption && app.appSettings.subtitles.displayMode !== 'source'
  const hasEditingCaption = hasEditingSourceCaption || hasEditingTranslationCaption
