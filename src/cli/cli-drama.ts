...
async function runShow(parsed: ParsedCliArgs): Promise<number> {
  ...
  const data: DramaProjectData = { project, chapters: store.listChapters(projectId), plan: store.getPlan(projectId), scripts: store.listScripts(projectId), assets: store.listAssets(projectId), storyboards: store.listStoryboards(projectId), generationTasks: store.listGenerationTasks(projectId), graphTemplates: store.listGraphTemplates() }
