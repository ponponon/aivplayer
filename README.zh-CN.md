...
Windows、Linux 和 macOS 正式安装包启动后会在后台检查 GitHub Releases；发现新版本时会先显示更新弹窗，用户可以选择跳过、稍后提醒或下载更新，下载完成后点击“重启并更新”才会退出并安装，不会强制中断当前播放或编辑。macOS 还可以从应用菜单的“检查更新…”手动触发检查。开发模式、Windows Store 和 `aivcli` 不参与自动更新。
...
这是旧版安装包没有把 LanceDB 的运行时依赖一起打包导致的启动问题，不是视频文件名、外置硬盘路径或 MP4 编码的问题；当前 `v0.6.1` 发布包已经包含修复。请直接下载对应 Release 的安装包，不要在应用包内手动安装 npm 依赖。

当前发布版本为 `v0.6.1`，请优先使用对应 Release 的安装包。
