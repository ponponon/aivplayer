...
export function useClipExportActions(model: AppModel, derived: AppDerived, syncPreferences: (length: ClipExportLengthSeconds, mode: ClipExportMode) => void) {
  const openClipExportDialog = (): void => {
    if (model.state.currentFile && !model.isExportingClip) model.setIsClipExportDialogOpen(true)
  }
  const confirmClipExport = async (selection: { durationSeconds: ClipExportLengthSeconds; mode: ClipExportMode }): Promise<void> => {
    ...
    try {
      const request: MediaClipExportRequest = { mediaPath: currentFile.path, startSeconds: model.videoRef.current?.currentTime ?? model.state.currentTime, durationSeconds: selection.durationSeconds, mode: selection.mode, subtitlePath: derived.subtitlePath ?? undefined, subtitleSrtPath: derived.subtitleSrtPath ?? undefined }
