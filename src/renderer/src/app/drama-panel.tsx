...
export function DramaPanel(): React.ReactElement {
  ...
  const createGenerationTask = (input: DramaGenerationTaskInput): void => {
    if (!selectedProjectId || busy) return
    setBusy(true)
    setError(null)
    void window.aiv.createDramaGenerationTask(selectedProjectId, input).then(() => refreshData()).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : String(reason))).finally(() => setBusy(false))
  }

  const cancelGenerationTask = (task: DramaGenerationTask): void => {
    if (!selectedProjectId || busy || !window.confirm(copy.generationCancelConfirm)) return
    setBusy(true)
    setError(null)
