...
- 现象：`align-self: stretch` 滚动修复已提交并通过 smoke，用户截图反馈「偏好设置还是不能下滑」，疑似修复无效。
- 原因：用户运行的是 `/Applications/AIVPlayer.app` 打包版（0.6.7，二进制构建于修复提交之前），仓库源码和 `out/` 构建里修复早已生效；用 Playwright 真实滚轮实测 `scrollTop` 正常变化，证明代码没问题，是运行实例过期。
- 经验：收到「bug 还在」的报告时，先核对对方实际运行的是哪个构建（打包版 / dev / out），比对二进制或产物的时间戳与修复提交时间，不要急着重改代码；smoke 通过后要提醒「需要重新打包安装才能吃到修复」。
- 处理：确认无需改码，引导重新打包安装；后续发布修复时尽量同步递增版本号，便于一眼区分新旧构建。
