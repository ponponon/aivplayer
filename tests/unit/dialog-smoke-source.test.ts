...
describe('dialog smoke source constraints', () => {
  ...
  it('uses stable selectors in the subtitle settings smoke script', () => {
    ...
    expect(smokeScript).toContain("dialogState.numberValues.includes('21')")
    expect(smokeScript).toContain("dialogState.selectValues.includes('relaxed')")
    expect(smokeScript).toContain("dialogState.selectValues.includes('source')")
