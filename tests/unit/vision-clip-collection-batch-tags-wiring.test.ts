...
describe('vision clip collection batch tags wiring', () => {
  it('keeps one normalized tag update behind shared IPC, desktop and preload', () => {
    ...
    expect(desktop).toContain('getClipInboxStore().updateCollectionsTags(collectionIds, tags)')
    ...
    expect(store).toContain('updateCollectionsTags(collectionIds: readonly string[], tags: unknown)')
