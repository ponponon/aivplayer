...
- 播放器底部控制栏提供“快速中文字幕”入口，不需要先打开 ASR 面板；一次点击会自动完成原文字幕生成、中文翻译和译文挂载，不遮挡视频画面。
- 已经存在原文字幕时，快速入口会直接跳过 ASR 生成，只执行翻译；命中中文原文或中文译文缓存时会显示就绪状态，避免重复计算。
- 快速中文字幕入口在 ASR 引擎未就绪时会把用户带到 ASR 工作区完成配置，避免点击后没有反馈。
- 支持 `⌘/Ctrl + Shift + C` 快捷键，在有媒体文件时直接执行快速中文字幕流程。
