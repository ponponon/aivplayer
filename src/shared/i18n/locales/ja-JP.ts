  export const jaJP = {
    ...
    editing: {
      subtitleQaTitle: '字幕 QA', subtitleQaIssueCount: (count: number) => count === 0 ? '問題なし' : `${count} 件`, subtitleQaClear: '明らかな字幕の問題はありません', subtitleQaJump: '移動', subtitleQaKindLabels: { empty: '空の字幕', overlap: '時間の重複', 'too-short': '短すぎる', 'too-long': '長すぎる', 'high-cps': '表示速度が速い', 'wide-line': '行幅が広すぎる', punctuation: '句読点', recognition: '認識エラーの疑い' }, subtitleQaMessage: (kind: string, value: number) => kind === 'overlap' ? `${value.toFixed(2)} 秒重複` : kind === 'high-cps' ? `${value.toFixed(1)} 文字/秒` : kind === 'wide-line' ? `約 ${value.toFixed(1)} em` : kind === 'too-short' || kind === 'too-long' ? `${value.toFixed(2)} 秒` : '字幕テキストを確認してください', waveformTrack: '波形', waveformUnavailable: '音声波形なし',
