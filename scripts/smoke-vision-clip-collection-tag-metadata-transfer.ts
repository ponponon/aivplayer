...
async function runSmoke(): Promise<void> {
  ...
  try {
    ...
    await page.getByText('已导入 2 个标签元数据，跳过 1 个未使用标签', { exact: true }).waitFor({ timeout: 10_000 })
    ...
    console.log(`AIVPlayer Smoke Vision Clip Collection Tag Metadata Transfer passed: ${JSON.stringify({ exportedCount: exportedManifest.metadata.length, importedCount: 2, skippedCount: 1, restored: true, tagsUnchanged: true, screenshotPath: screenshotPath ?? null })}`)
  } finally {
