...
이 문제는 이전 설치 파일이 LanceDB 런타임 의존성을 함께 패키징하지 않아 발생한 시작 오류입니다. 동영상 파일명, 외장 드라이브 경로 또는 MP4 인코딩이 원인이 아닙니다. 현재 `v0.5.6` 릴리스에는 수정 사항이 포함되어 있습니다. 해당 Release의 설치 파일을 다운로드하고 앱 번들 안에 npm 의존성을 직접 설치하지 마세요.

현재 릴리스는 `v0.5.6`이므로 해당 Release의 설치 파일을 우선 사용하세요.
