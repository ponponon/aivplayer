...
describe('release workflow source constraints', () => {
  ...
  it('keeps platform builds separate from release publishing', () => {
    ...
    expect(releaseWorkflow).toContain('--prepackaged release/linux-arm64-unpacked')
  ...
  it('builds, smoke-tests and publishes the ARM64 Snap before checking ARM64 release artifacts', () => {
    ...
    expect(snapArm64Workflow).toContain('name: Smoke test Snap CLI launcher')
    expect(snapArm64Workflow).toContain('sudo snap install --dangerous "$snap_file"')
    expect(snapArm64Workflow).toContain('timeout 30s snap run aivplayer --cli --version')
    ...
    expect(snapArm64Workflow.indexOf('name: Smoke test Snap CLI launcher')).toBeLessThan(snapArm64Workflow.indexOf('name: Publish verified Snap'))
  ...
  it('waits for all desktop artifacts before creating a release', () => {
    expect(releaseWorkflow).toContain('needs: [build-macos, build-windows, build-windows-arm64, build-linux, build-linux-arm64, build-snap-x64, build-snap-arm64]')
  ...
  it('stages platform runtimes before packaging', () => {
    ...
    expect(releaseWorkflow).toContain('autobuild-2026-08-25-13-06/ffmpeg-n8.1.2-44-g7c533d0f86-winarm64-gpl-8.1.zip')
    expect(releaseWorkflow).not.toContain('releases/download/latest/ffmpeg-n8.1-latest-winarm64-gpl-8.1.zip')
    expect(releaseWorkflow).toContain('57b7dc944c03991067db38a25976147db127597100f56c10fb3c8b537ff30dcc')
