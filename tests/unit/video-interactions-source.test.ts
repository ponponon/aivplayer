...
describe('video surface interaction source constraints', () => {
  it('separates single-click playback from double-click fullscreen', () => {
    ...
    expect(playerCss).toMatch(/\.stage:fullscreen\s*\{[^}]*display:\s*grid;[^}]*grid-template-rows:\s*minmax\(0,\s*1fr\)\s+auto;/s)
    expect(playerCss).toMatch(/\.stage:fullscreen\s+\.video-frame\s*\{[^}]*position:\s*relative;[^}]*inset:\s*auto;/s)
    expect(playerCss).toMatch(/\.stage:fullscreen\s*>\s*\.subtitle-overlay:not\(\.empty\)\s*\{[^}]*grid-row:\s*1;[^}]*grid-column:\s*1;/s)
    expect(playerCss).toMatch(/\.stage:fullscreen\s+\.control-deck\s*\{[^}]*position:\s*relative;[^}]*grid-row:\s*2;[^}]*margin:/s)
