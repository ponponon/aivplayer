...
export function VideoSurface(): React.ReactElement {
  ...
  return <><video key={mediaUrl || 'empty-media'} ref={app.videoRef} className={`video-surface ${isPunchIn ? 'is-punch-in' : ''} ${compactFramingTreatment ? `is-framing-${compactFramingTreatment}` : ''} ${activeSplitPosition ? `is-${activeSplitPosition}` : ''} ${personMattePreviewFrameUrl ? 'is-person-matte' : ''}`} data-framing-treatment={framingResolution?.state.treatment ?? 'full'} data-testid={personMattePreviewFrameUrl ? 'editing-person-matte-preview' : undefined} style={videoStyle} src={mediaUrl} preload="metadata" onClick={app.handleVideoClick} onDoubleClick={app.handleVideoDoubleClick} onPlay={(event) => app.syncPlaybackState(event.currentTarget)} onPlaying={(event) => { app.syncPlaybackState(event.currentTarget); app.clearPlaybackError() }} onCanPlay={(event) => { app.syncPlaybackState(event.currentTarget); app.clearPlaybackError() }} onPause={(event) => { const currentTime = event.currentTarget.currentTime; app.syncPlaybackState(event.currentTarget); if (!app.isEditingMode) app.persistPlaybackProgress(currentTime, true) }} onEnded={(event) => { app.editingResumePlaybackRef.current = false; if (app.isEditingMode) { app.playbackEndedRef.current = true; app.syncPlaybackState(event.currentTarget); return }; app.handlePlaybackEnded() }} onLoadedMetadata={onLoadedMetadata} onTimeUpdate={(event) => { const currentTime = event.currentTarget.currentTime; app.syncPlaybackState(event.currentTarget); app.setState((current) => ({ ...current, currentTime, error: null })); if (!app.isEditingMode) app.persistPlaybackProgress(currentTime) }} onVolumeChange={(event) => { const { volume, muted } = event.currentTarget; app.setState((current) => ({ ...current, volume, muted })) }} onError={(event) => { app.syncPlaybackState(event.currentTarget); app.handleMediaError(event) }} controls={false} />{personMatteTrackProgress?.status === 'processing' ? <div className="editing-person-matte-track-progress" data-testid="editing-person-matte-track-progress" role="status" aria-live="polite"><span>{copy.editing.personMatteProcessing(personMatteTrackProgress.processedFrames, personMatteTrackProgress.totalFrames)}</span><strong>{progressPercent}%</strong><i><b style={{ width: `${progressPercent}%` }} /></i></div> : null}</>
}
