...
- 经验：播放状态必须以当前媒体元素的 `!paused && !ended` 为唯一事实来源，所有事件和 `play()` / `pause()` Promise 都走同一个同步函数；切换媒体前先暂停，并按媒体 URL 更换 `<video>` 实例隔离旧事件。任何独立的预览播放器（例如剪辑导出预览）也必须复用相同规则，不能再维护一套只由按钮动作驱动的布尔状态。
