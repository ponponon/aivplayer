...
import { isSubtitleQaRepairableIssue, type SubtitleQaIssue } from '../../../shared/subtitle-qa'
...
type Props = {
  ...
  onRepair: (issues: readonly SubtitleQaIssue[]) => void
}

export function EditingSubtitleQa({ issues, copy, onSeek, onRepair }: Props): React.ReactElement {
  const repairableIssues = issues.filter(isSubtitleQaRepairableIssue)
  return <details className="editing-subtitle-qa" data-testid="editing-subtitle-qa">
    ...
    <div className="editing-subtitle-qa-panel">
      {repairableIssues.length > 0 ? <div className="editing-subtitle-qa-repair"><span>{copy.subtitleQaRepairableIssueCount(repairableIssues.length)}</span><button className="editing-tool-button" type="button" onClick={() => onRepair(repairableIssues)} data-testid="subtitle-qa-repair">{copy.subtitleQaRepair}</button></div> : null}
