...
describe('dialog smoke source constraints', () => {
  it('exposes the clip export and media details smoke scripts from package scripts', () => {
    ...
    expect(packageJson.scripts?.['smoke:all']).toBe(
      'npm run smoke:settings-dialog:all && npm run smoke:dialogs:all && npm run smoke:subtitle-settings && npm run smoke:translation && npm run smoke:translation:player && npm run smoke:playback-history && npm run smoke:playback-segments && npm run smoke:structure-analysis && npm run smoke:vision-clip-inbox && npm run smoke:open-video'
    )
  })
