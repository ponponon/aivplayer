...
export function VideoSurface(): React.ReactElement {
  ...
  const transformOrigin = framingResolution?.isTransitioning ? '50% 50%' : punchInOrigin === 'left' ? '0% 50%' : punchInOrigin === 'right' ? '100% 50%' : '50% 50%'
  ...
  const punchTransform = framingResolution?.isTransitioning
    ? `translateX(${framingResolution.translateXPercent}%) scale(${framingResolution.scale})`
    : isPunchIn
