...
describe('video surface interaction source constraints', () => {
  it('separates single-click playback from double-click fullscreen', () => {
    ...
    expect(appSource).toContain('const fullscreenState = getFullscreenControlState(document.fullscreenElement, app.fullscreenRef.current)')
    expect(appSource).toContain('fullscreenState.isActive ? <Minimize2 size={16} /> : <Fullscreen size={16} />')
    ...
    expect(appSource).toContain('else if (fullscreenTarget) await fullscreenTarget.requestFullscreen()')
    expect(appSource).toContain("if (event.key === 'Escape')")
    expect(appSource).toContain('void document.exitFullscreen()')
    expect(appSource).toContain('getFullscreenControlState(document.fullscreenElement, model.fullscreenRef.current).isActive')
    expect(appSource).toContain("document.addEventListener('fullscreenerror', onFullscreenChange)")
    expect(playerCss).toMatch(/\.video-surface:fullscreen\s*\{[^}]*object-fit:\s*contain;/s)
    expect(playerCss).toContain('.stage:fullscreen .control-deck')
  })
})
