...
const copy = {
  'zh-CN': {
    ...
    hero: { eyebrow: '本地优先 · 为视频而生', title: '你的本地 AI<br><em>视频工作台。</em>', lede: 'AIVPlayer 把播放、字幕、视觉检索和剪辑收进一款专注的桌面应用。媒体和项目默认留在你的设备上。', primary: '下载 AIVPlayer', secondary: '查看功能', localNote: '本地运行，无需账号。', mediaCaption: '真实界面 · 截取自 AIVPlayer 0.6.2' },
    ...
    preview: { eyebrow: '不是占位图', title: '先看看工作区<br><em>真实的样子。</em>', lede: '以下都是桌面端真实截图，不是占位符。界面深色、适合键盘操作，并围绕当前媒体展开。', playbackAlt: 'AIVPlayer 本地视频播放工作区', playerAlt: 'AIVPlayer 显示视频和字幕生成面板', visualAlt: 'AIVPlayer 视觉媒体库与本地搜索控件', editingAlt: 'AIVPlayer 视频剪辑时间线、胶片条和剪辑工具', imageAlt: 'AIVPlayer 图片工作区与裁剪导出控件', settingsAlt: 'AIVPlayer 偏好设置窗口，显示界面语言设置', playerTitle: '播放 + 字幕工作区', playerCaption: '观看、跳转、检查运行状态，并在一个界面准备本地 ASR。', visualTitle: '视觉媒体库', visualCaption: '使用 SigLIP2 搜索本地视频画面，直接跳到你记得的那个瞬间。', editingTitle: '视频剪辑时间线', editingCaption: '在本地时间线上裁剪、分割、编排并准备导出视频。', imageTitle: '图片工作区', imageCaption: '无需离开桌面应用，即可调整、缩放、转换和导出图片。', settingsTitle: '尊重你的使用方式', settingsCaption: '语言、文件夹、播放、截图录屏、快捷键和 AI 服务都可以配置。' },
    ...
    download: { eyebrow: '准备好时再开始', title: '为你的电脑下载 AIVPlayer。', lede: '为你的系统选择安装包，应用完全在本地运行——不需要账号，不会上传文件。', recommendedLabel: '为这台电脑推荐', downloadRecommended: '下载推荐版本', downloadFor: '下载 {platform} 版', chooserLabel: '或选择其他平台', architectureLabel: '构建', formatLabel: '格式', historyVersionLabel: '版本', versionLatest: '最新版本（{version}）', downloadSelected: '下载这个构建', archiveBody: '更早的版本保留在 GitHub Releases。', archiveLink: '查看全部版本', detected: '已检测到', unavailable: '该架构目前没有安装包。', architectureUnknown: '浏览器无法可靠确认架构，请手动选择。', choosePlatform: '请在下方选择平台开始下载。', chooseArchitecture: '选择平台和架构', unavailableTag: '暂无', macos: 'macOS', windows: 'Windows', linux: 'Linux', arm64: 'ARM64', x64: 'x64', privacyBody: '你的文件、工程、字幕和索引默认保存在本地。', privacyLink: '阅读完整隐私政策', arch: { darwin: { arm64: 'Apple 芯片', x64: 'Intel' }, win32: { x64: '标准版（x64）', arm64: 'ARM64' }, linux: { x64: 'x64', arm64: 'ARM64' } } },
  ...
  'en-US': {
    ...
    preview: { eyebrow: 'NO MOCKUPS', title: 'See the workspace<br><em>as it really is.</em>', lede: 'These are direct captures from the desktop app, not placeholders. The interface is dark, keyboard-friendly, and designed around the media in front of you.', playbackAlt: 'AIVPlayer local video playback workspace', playerAlt: 'AIVPlayer showing a video and subtitle generation panel', visualAlt: 'AIVPlayer visual media library with local search controls', editingAlt: 'AIVPlayer video editing timeline with filmstrip and editing tools', imageAlt: 'AIVPlayer image workspace with crop and export controls', settingsAlt: 'AIVPlayer preferences window showing interface language settings', playerTitle: 'Playback + subtitle workspace', playerCaption: 'Watch, seek, inspect engine readiness, and prepare local ASR in one view.', visualTitle: 'Visual media library', visualCaption: 'Search local video frames with SigLIP2 and jump to the moment you remember.', editingTitle: 'Video editing timeline', editingCaption: 'Trim, split, arrange, and prepare a video for export on a local timeline.', imageTitle: 'Image workspace', imageCaption: 'Adjust, resize, convert, and export images without leaving the desktop app.', settingsTitle: 'Preferences that respect your setup', settingsCaption: 'Language, folders, playback, capture, shortcuts, and AI services stay configurable.' },
  ...
  'ja-JP': {
    ...
    hero: { eyebrow: 'ローカル優先 · 映像のために', title: 'あなたのローカル AI<br><em>ビデオワークスペース。</em>', lede: 'AIVPlayer は再生、字幕、ビジュアル検索、編集をひとつのデスクトップアプリにまとめます。メディアとプロジェクトは標準で端末内に保存されます。', primary: 'AIVPlayer をダウンロード', secondary: '機能を見る', localNote: 'ローカルで動作。アカウント不要。', mediaCaption: '実際の画面 · AIVPlayer 0.6.2 で撮影' },
    ...
    preview: { eyebrow: 'モックアップではありません', title: 'ワークスペースを<br><em>実際の画面で。</em>', lede: 'デスクトップアプリを直接撮影した画面です。暗いテーマ、キーボード操作、目の前のメディアを中心に設計されています。', playbackAlt: 'AIVPlayer のローカル動画再生ワークスペース', playerAlt: '動画と字幕生成パネルを表示した AIVPlayer', visualAlt: 'ローカル検索コントロール付き AIVPlayer ビジュアルメディアライブラリ', editingAlt: 'フィルムストリップと編集ツールを表示した AIVPlayer の動画編集タイムライン', imageAlt: 'クロップと書き出しコントロール付き AIVPlayer の画像ワークスペース', settingsAlt: 'インターフェース言語設定を表示した AIVPlayer の環境設定', playerTitle: '再生 + 字幕ワークスペース', playerCaption: '再生、シーク、エンジン状態の確認、ローカル ASR の準備をひとつの画面で。', visualTitle: 'ビジュアルメディアライブラリ', visualCaption: 'SigLIP2 でローカル動画のフレームを検索し、覚えている瞬間へ移動できます。', editingTitle: '動画編集タイムライン', editingCaption: 'ローカルタイムラインで動画をトリミング、分割、配置して書き出しを準備します。', imageTitle: '画像ワークスペース', imageCaption: 'デスクトップアプリを離れずに、画像の調整、リサイズ、変換、書き出しができます。', settingsTitle: '使い方に合わせる環境設定', settingsCaption: '言語、フォルダ、再生、キャプチャ、ショートカット、AI サービスを設定できます。' },
  ...
  'ko-KR': {
    ...
    hero: { eyebrow: '로컬 우선 · 영상을 위해 설계', title: '당신의 로컬 AI<br><em>비디오 워크스페이스.</em>', lede: 'AIVPlayer는 재생, 자막, 시각 검색, 편집을 하나의 집중된 데스크톱 앱으로 묶습니다. 미디어와 프로젝트는 기본적으로 기기에 보관됩니다.', primary: 'AIVPlayer 다운로드', secondary: '기능 살펴보기', localNote: '로컬에서 실행됩니다. 계정이 필요 없습니다.', mediaCaption: '실제 인터페이스 · AIVPlayer 0.6.2에서 캡처' },
    ...
    preview: { eyebrow: '목업이 아닙니다', title: '워크스페이스를<br><em>있는 그대로 보세요.</em>', lede: '데스크톱 앱에서 직접 캡처한 화면입니다. 어둡고 키보드 친화적이며 지금 보고 있는 미디어를 중심으로 설계되었습니다.', playbackAlt: 'AIVPlayer 로컬 비디오 재생 워크스페이스', playerAlt: '영상과 자막 생성 패널을 보여주는 AIVPlayer', visualAlt: '로컬 검색 컨트롤이 있는 AIVPlayer 시각 미디어 라이브러리', editingAlt: '필름스트립과 편집 도구를 보여주는 AIVPlayer 비디오 편집 타임라인', imageAlt: '자르기 및 내보내기 컨트롤이 있는 AIVPlayer 이미지 워크스페이스', settingsAlt: '인터페이스 언어 설정을 보여주는 AIVPlayer 환경설정', playerTitle: '재생 + 자막 워크스페이스', playerCaption: '재생하고 탐색하고 엔진 상태를 확인하며 로컬 ASR을 한 화면에서 준비합니다.', visualTitle: '시각 미디어 라이브러리', visualCaption: 'SigLIP2로 로컬 비디오 프레임을 검색하고 기억하는 순간으로 바로 이동합니다.', editingTitle: '비디오 편집 타임라인', editingCaption: '로컬 타임라인에서 영상을 자르고, 나누고, 배치하고, 내보낼 준비를 합니다.', imageTitle: '이미지 워크스페이스', imageCaption: '데스크톱 앱을 벗어나지 않고 이미지를 조정하고, 크기를 바꾸고, 변환하고, 내보냅니다.', settingsTitle: '사용 방식에 맞는 환경설정', settingsCaption: '언어, 폴더, 재생, 캡처, 단축키, AI 서비스를 설정할 수 있습니다.' },
...
function orderDownloadFormats(platform, formats) {
  const priority = DOWNLOAD_FORMAT_PRIORITY[platform] ?? []
...
function getFormatOptions(platform, architecture, release = getCurrentDownloadRelease()) {
  ...
  return orderDownloadFormats(platform, Object.keys(asset))
}
...
function renderDownloadControls(locale) {
  ...
  if (recLink) {
    ...
    const recommendationActionKey = recommended.asset
      ? 'download.downloadRecommended'
      : detectedDownloadTarget.platform && !recommended.architecture
        ? 'download.chooseArchitecture'
        : 'download.choosePlatform'
    recLink.setAttribute('aria-label', getValue(locale, recommendationActionKey))
    if (recLabel) {
      recLabel.textContent = recommended.platform
        ? recommended.asset
          ...
          : getValue(locale, recommendationActionKey)
        : getValue(locale, recommendationActionKey)
    }
  }
...
function revealDownloadChooser() {
  chooserExpanded = true
  renderDownloadControls(detectLocale())
  requestAnimationFrame(() => {
    const detail = document.getElementById('download-detail')
    const architectureChip = detail?.querySelector('#download-architecture-chips .download-chip:not(.is-disabled)')
    const firstChoice = architectureChip
      ?? detail?.querySelector('.download-chip:not(.is-disabled)')
      ?? document.querySelector('#download-platform-grid .download-platform-card')
    detail?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    firstChoice?.focus({ preventScroll: true })
  })
}

// If the recommended build is unavailable, expand and reveal the chooser instead of going to GitHub
