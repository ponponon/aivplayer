...
const FALLBACK_DOWNLOAD_MANIFEST = {
  ...
  releases: [{
    ...
    version: '0.5.4',
    tag: 'v0.5.4',
    githubUrl: `${GITHUB_RELEASES_URL}/tag/v0.5.4`,
    assets: {
      'darwin-arm64': { name: 'AIVPlayer-0.5.4-arm64.dmg', url: createGithubAssetUrl('0.5.4', 'AIVPlayer-0.5.4-arm64.dmg'), format: 'dmg' },
      'win32-x64': { name: 'AIVPlayer-Setup-0.5.4-x64.exe', url: createGithubAssetUrl('0.5.4', 'AIVPlayer-Setup-0.5.4-x64.exe'), format: 'exe' },
      'win32-arm64': { name: 'AIVPlayer-Setup-0.5.4-arm64.exe', url: createGithubAssetUrl('0.5.4', 'AIVPlayer-Setup-0.5.4-arm64.exe'), format: 'exe' },
      'linux-x64': { name: 'aivplayer-0.5.4-x86_64.AppImage', url: createGithubAssetUrl('0.5.4', 'aivplayer-0.5.4-x86_64.AppImage'), format: 'AppImage' },
      'linux-arm64': { name: 'aivplayer-0.5.4-arm64.AppImage', url: createGithubAssetUrl('0.5.4', 'aivplayer-0.5.4-arm64.AppImage'), format: 'AppImage' }
    }
  }]
}
...
const copy = {
  'zh-CN': {
    ...
    download: { eyebrow: '准备好时再开始', title: '从正确的版本开始。', lede: '当前版本会自动推荐；上一版本单独提供下载，需要指定平台和架构时可以展开手动选择。', recommendedLabel: '为这台设备推荐', downloadRecommended: '下载推荐版本', noDownload: '查看可用版本', historyKicker: '历史版本', historyTitle: '下载上一版本。', historyBody: '上一版本提供直接下载；更早版本保留在 GitHub Releases。', historyVersionLabel: '版本', downloadHistory: '下载历史版本', status: '版本清单已就绪。', retentionLabel: '快速下载', manualTitle: '手动选择平台和架构', manualBody: '如果你要给另一台电脑下载，或需要指定架构，可以在这里选择。', platformLabel: '平台', architectureLabel: '架构', downloadSelected: '下载所选版本', archiveBody: '更早版本保留在 GitHub Releases。', archiveLink: '查看全部版本', detected: '检测到', release: '版本', unavailable: '该架构当前没有安装包，请查看 GitHub Releases。', macos: 'macOS', windows: 'Windows', linux: 'Linux', arm64: 'ARM64', x64: 'x64', privacyTitle: '本地优先的设计', privacyBody: '你的文件、工程、字幕和索引默认保存在本地。', privacyLink: '查看完整隐私政策' },
  ...
  'ko-KR': {
    ...
    download: { eyebrow: '준비되었을 때', title: '맞는 빌드로 시작하세요.', lede: '현재 버전은 자동으로 추천합니다. 이전 버전은 별도 다운로드로 제공하며, 아래에서 플랫폼과 아키텍처를 직접 선택할 수 있습니다.', recommendedLabel: '이 컴퓨터에 권장', downloadRecommended: '권장 빌드 다운로드', noDownload: '사용 가능한 버전 보기', historyKicker: '이전 버전', historyTitle: '이전 버전 다운로드.', historyBody: '이전 릴리스는 직접 다운로드할 수 있습니다. 더 오래된 릴리스는 GitHub에 남아 있습니다.', historyVersionLabel: '버전', downloadHistory: '이전 버전 다운로드', status: '릴리스 목록을 불러왔습니다.', retentionLabel: '빠른 다운로드', manualTitle: '플랫폼과 아키텍처 직접 선택', manualBody: '다른 컴퓨터용으로 받거나 특정 아키텍처가 필요할 때 사용하세요.', platformLabel: '플랫폼', architectureLabel: '아키텍처', downloadSelected: '선택한 빌드 다운로드', archiveBody: '이전 버전은 GitHub Releases에 계속 보관됩니다.', archiveLink: '모든 릴리스 보기', detected: '감지됨', release: '릴리스', unavailable: '이 아키텍처용 설치 프로그램이 아직 없습니다. GitHub Releases에서 사용 가능한 빌드를 확인하세요.', macos: 'macOS', windows: 'Windows', linux: 'Linux', arm64: 'ARM64', x64: 'x64', privacyTitle: '로컬 우선 설계', privacyBody: '파일, 프로젝트, 자막, 인덱스는 기본적으로 로컬에 남습니다.', privacyLink: '전체 개인정보 처리방침 보기' },
...
function pickDownloadTarget(preferred = detectedDownloadTarget, release = getCurrentDownloadRelease()) {
  ...
  const architecture = preferred.architecture && architectures.includes(preferred.architecture)
...
function renderDownloadControls(locale) {
  ...
  if (recommendedLink) {
    recommendedLink.href = recommended.asset?.url ?? release.githubUrl ?? GITHUB_RELEASES_URL
    recommendedLink.setAttribute('aria-label', recommended.asset ? getValue(locale, 'download.downloadRecommended') : getValue(locale, 'download.noDownload'))
    if (recommendedLabel) recommendedLabel.textContent = getValue(locale, recommended.asset ? 'download.downloadRecommended' : 'download.noDownload')
  }
  if (manualLink) {
...
document.addEventListener('DOMContentLoaded', () => {
  detectedDownloadTarget = detectDownloadTarget()
