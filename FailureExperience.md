## 2026-09-01：嵌套设置容器不能只声明 grid-column

- 现象：字幕设置页的本地 TTS provider 中，多个设置卡片边框贴在一起，看起来像 CSS 的 `gap` 失效。
- 原因：外层设置卡片的 `gap` 只负责排列它的直接子项；TTS 子组件内部的包装 `<div>` 只有 `grid-column`，没有 `display: grid` 和 `gap`，内部字段因此按普通块级元素连续排列。
- 经验：凡是把多个设置块封装进一个组件，都必须让包装容器显式建立布局上下文，并统一设置 `min-width: 0`、`align-content: start` 和间距；不能把父级 Grid 的 `gap` 当成所有嵌套层级的间距。
- 处理：增加可复用的 `settings-stack` 契约，TTS provider 使用该契约，源码测试和设置弹窗 smoke 同时检查计算后的 Grid、`row-gap` 及每个子块的实际像素间隔。

## 2026-09-01：托管服务入口不能用 IP 地理位置硬编码

- 现象：同一个用户可能在大陆、海外、VPN 和移动网络之间切换；只按客户端 IP 判断地区会遇到 VPN 出口、移动运营商地址变化、地理库滞后和跨境网络实际不可达等误判，导致入口选错或切换不及时。
- 经验：桌面客户端无法可靠证明用户的真实地理位置，也没有必要把“用户在哪里”当作路由前提；真正需要判断的是当前网络栈访问每个入口的健康状态、延迟和最近业务请求结果。健康探测必须不携带字幕正文、API Key 或 IP 地理服务依赖。
- 处理：托管翻译路由改用同一套动态代理 / Electron 系统网络栈分别探测全球和大陆入口，保存短期延迟样本并以中位数排序；延迟接近时稳定优先大陆入口，质量变化连续确认后才切换，业务请求网络错误或 5xx 立即熔断并尝试另一入口。VPN 开关、换网和跨国移动通过定时刷新与失败失效自动重新收敛。
...
- 原因：Electron 同时存在 Node HTTP 网络栈和 Chromium 网络栈，Node 的全局代理设置不等于全局 `fetch` 的动态代理配置；并且 `process.env` 是进程启动时的环境快照，外部 VPN 软件改变代理端口后，已运行进程不会自动得到新的环境变量。
- 经验：桌面应用需要把“系统路由 / TUN VPN”和“本地 HTTP / SOCKS5 代理”分开处理：默认请求使用 Electron `net.fetch`，显式代理则使用独立 Session 的 `setProxy`；代理端口必须按请求周期重新发现，不能只在应用启动时读取一次，也不能把用户 API Key 放进探测请求或日志。
- 处理：翻译与总结共用动态网络传输层，优先探测当前环境代理和 QuickQ 动态监听端口，代理不可用时回退系统网络；每次路由刷新只发送无密钥的 HEAD 探测，实际业务请求失败后立即重新选择路径。

## 2026-08-28：非阻塞提示不能用占位布局制造整行空白

- 现象：为避免 AI 工作流浮层遮挡功能，把状态条放入 AppShell 的独立 Grid 行后，播放器整体下移，状态条左侧出现大块空白，界面显得突兀。
- 原因：这是把“不能拦截底层点击”和“必须为浮层预留布局空间”混成了同一个问题；状态提示本身不需要参与文档流，强行增加 Grid 行会改变主界面高度。
- 经验：非阻塞通知应采用固定定位加 `pointer-events: none`，再只给真正的交互控件恢复 `pointer-events: auto`；这样既不抢占布局，也不阻断底层功能。
- 处理：移除 AI 工作流布局占位行，恢复顶部悬浮定位；状态条外壳穿透鼠标，操作按钮保留点击能力，并补充“无占位行 + 点击穿透”的源码回归测试。

## 2026-08-28：异步字幕缓存不能清空刚生成的内容总结

- 现象：内容总结完成后，面板会在已有总结和“还没有内容总结”之间闪烁；任务中心已经显示总结任务完成，但总结面板偶尔又回到空态。
- 原因：原文总结完成后，翻译字幕缓存可能晚一步恢复，`summarySourcePath` 从原文切到译文；缓存 effect 将这个正常的首选来源升级误判为总结失效并先清空结果，导致明显的 article → empty-state 闪烁。
- 经验：异步恢复派生数据时，不能因为首选来源变化就抹掉仍然可用的结果；应保留当前结果并后台尝试加载更优来源，只有源文件版本、目标语言、模型或模式确实失效时才清除。
- 处理：内容总结面板改为互斥渲染文章 / loading / 空态；原文总结在译文缓存探测期间保持可见，译文总结缓存命中后再无空白切换，并补充源版本回归测试。
...
## 2026-08-28：配置类设置必须按用户任务组织，而不是按数据字段堆叠

- 现象：AI 服务设置页同时展示“当前使用的服务”“配置档案”和编辑字段，但缺少清晰的阅读顺序；档案列表只有连续文本，编辑表单被追加在页面底部，专业用户需要反复确认当前状态，普通用户更难判断从哪里开始配置。
- 原因：界面沿用了通用 `SettingsField` 的线性堆叠方式，没有把“选择服务”“管理已保存配置”“填写自定义服务”这三个不同任务拆成独立的交互区域；新增配置还会在点击按钮时立即写入一个空白档案，取消操作可能留下半成品。
- 经验：配置页面应先回答“当前生效的是什么”，再提供“可用配置有哪些”，最后通过有标题、有说明、有保存边界的编辑区承载复杂字段；状态、动作和配置详情要在同一条视觉层级内呈现，避免仅依赖字段名称让用户猜测流程。
- 处理：AI 服务页改为当前服务摘要、配置档案卡片、自定义服务编辑区和测试结果四段结构；档案增加清晰的使用中 / 已配置 / 待完成状态和直接切换动作；新档案只保存在草稿状态，保存时才写入设置并设为当前服务，取消不会产生空档案；补充四语说明和接口、模型、Key 占位提示。

## 2026-08-28：管理型设置不能继续堆叠大卡片

- 现象：上一版虽然修正了保存边界和状态提示，但 AI 服务页仍然同时出现介绍卡、当前服务卡、配置档案卡和页面内编辑卡；信息重复、视觉层级过多，专业用户也难以快速判断配置入口。
- 原因：把“管理服务”和“编辑服务”放在同一条页面流中，沿用了设置卡片的通用布局，没有按管理型页面的任务拆分信息密度。
- 经验：服务管理页应优先采用“页面标题 + 一张列表 + 行级状态 / 操作”的结构；新增和编辑属于高认知负担任务，应放进独立、可聚焦的弹窗，而不是继续追加到列表下方。参考其他产品时只吸收信息架构，不复制视觉资产。
- 处理：AI 服务页改为紧凑服务管理列表，统一展示服务、配置、状态和操作；当前服务用摘要条突出，托管服务说明默认折叠；新增 / 编辑使用带焦点陷阱的独立弹窗，支持 Esc、点击遮罩关闭，且取消不会写入设置；补充四语言文案、响应式布局和 Electron Smoke 断言。

## 2026-08-28：简化列表时不能移除最小视觉边界

- 现象：AI 服务页虽然已改成单列表，但列表区域完全没有圆角矩形边界，看起来像散落在管理容器里的裸表格，与其他设置控件不一致。
- 原因：为了减少卡片堆叠，重构时把 `.ai-service-table` 的完整边框和圆角误删，只保留上下分隔线。
- 经验：减少层级不等于取消边界；管理列表仍应保留轻量的 `border + border-radius` 容器，用来表达列表范围，不要给每一行重复堆大卡片。
- 处理：为 AI 服务列表恢复基于现有设计 token 的圆角矩形边界，并补充源码约束与 Electron Smoke 的计算样式断言。

## 2026-08-28：视觉回归不能只靠人工记忆

- 现象：同一类 UI 问题多次出现。功能测试和类型检查都通过了，但重构后仍然遗漏圆角、边界或布局细节；经验文档虽然持续增加，却没有阻止错误再次发生。
...
## 2026-08-31：TSX 组件返回类型要遵循项目 React 类型配置

- 现象：新增集合历史详情状态组件后，`npm run typecheck` 报 `Cannot find namespace 'JSX'`。
- 原因：项目当前 React / TypeScript 配置没有提供全局 `JSX` 命名空间，组件显式写 `JSX.Element` 会脱离现有类型导入约定。
- 经验：新增 TSX 组件前先检查项目已有 React 类型写法；优先使用 `ReactNode` 等从 `react` 导入的类型，避免依赖全局命名空间。
- 处理：将返回类型改为从 `react` 导入的 `ReactNode`，重新通过类型检查、构建和真实 Electron Smoke。

## 2026-08-31：逐条撤销不能只按历史 ID 执行快照

- 现象：新增指定历史撤销后，较早的收藏 / 归档记录可以在后续 flags 操作之后直接恢复旧状态，覆盖用户当前的后续修改。
- 原因：原有 flags 撤销只保存并恢复“操作前 flags”，没有像重命名、内容编辑、删除和复制那样校验集合当前是否仍匹配“操作后快照”；全局最近一次入口暂时没有暴露这个问题，但逐条入口扩大了可选目标范围。
- 经验：把“最近一次操作”扩展为“任意历史条目”时，必须重新审计每一种操作类型的状态前置条件；历史 ID 只能定位记录，不能代替当前数据版本校验。
- 处理：逐条撤销 / 重做要求记录状态符合目标方向；flags 增加 `isFavorite`、`isArchived` 和 `updatedAt` 快照匹配，冲突时保持当前数据不变并返回不可操作提示；补充独立条目、冲突和状态转换测试。

## 2026-09-01：原生 select 弹窗会绕过应用的视觉系统

- 现象：AI 服务设置中的入口选择器打开后显示 macOS 原生列表；同类原生 `<select>` 还分散在播放控制、影视库、视觉检索、剪辑和 Web 面板中，导致间距、颜色、圆角、层级和键盘行为无法统一。
- 原因：只给部分选择器补 CSS 不能改变浏览器 / Electron 对 `<select>` 展开列表的绘制方式，设置弹窗内的原生 popup 也不受应用样式和 Portal 层级控制；同时把原生事件类型直接传给自定义控件会留下 `selectedOptions` 和多选语义差异。
- 经验：需要品牌化列表时必须替换控件语义，而不是继续覆盖原生 `<select>` 的外观；统一组件要集中处理单选 / 多选、禁用项、键盘导航、Esc、点击外部关闭、焦点恢复和长列表定位，并提供与业务现有 `onChange` 兼容的最小事件对象。替换后要全局扫描 `<select>`，并用真实 Electron 打开菜单验证 Portal、层级和滚动。
- 处理：新增共享 `AppSelect`，桌面端通过 Portal 渲染固定定位菜单，Web 端复用同一交互和专属样式；所有 renderer / web 原生选择器与 Smoke 的 `.selectOption()` 调用同步迁移，并加入无原生 select、菜单定位、Esc 关闭和截图证据检查。

## 2026-09-01：Logo 探索不能默认套用可爱 IP 风格

- 现象：首批 Logo 候选使用了过强的拟人化、粉彩配色和卡通表情，用户反馈整体难看，不符合 AIVPlayer 的专业视频工作台定位。
- 原因：只根据“IP Logo”生成规则强调可爱和角色化，没有优先校准产品已有的深色影院界面、媒体工具属性和成熟用户预期。
- 经验：品牌视觉探索必须先锁定产品气质，再决定是否使用角色化表达；专业桌面工具应优先验证轮廓、缩小识别度、色彩层级和跨平台图标适配，不能把“可爱”当成默认审美。
- 处理：改用深色影院背景、播放符号和圆润几何层叠的克制路线；先生成候选供用户选择，再将选定版本同步到主图标、官网图标、Linux 图标、ICNS 和 ICO，避免只更新单个平台资源。

## 2026-09-02：macOS Dock 图标不能把品牌原图直接当作展示资源

- 现象：更新品牌图标后，macOS Dock 中的图标比旁边 App 更大，外框和圆角也不一致；同一份资源同时被开发态 Dock 和正式 macOS 包使用。
- 原因：品牌原图是满画布的不透明正方形，开发态通过 Electron `app.dock.setIcon()` 直接读取 PNG，正式包又把同一份满画布图写进 ICNS；没有区分“跨平台品牌源图”和“macOS 展示版图标”的留白、圆角和阴影。
- 经验：macOS 图标必须分开验证源图、ICNS 和 Electron Dock PNG；开发态 `setIcon()` 不会替应用补齐统一的视觉留白，不能只替换品牌图片后假设 Dock 会自动缩放到和其它 App 一致。
- 处理：保留 `brand/icon.png` 作为跨平台源图，增加带 macOS 标准内缩轮廓的 `brand/icon-macos.png`；开发态 Dock 和打包后的 macOS 运行时改用该展示资源，Windows / Linux / Web 继续使用各自的原始资源，并对 ICNS 做实际 macOS 解码验证。

## 2026-09-02：品牌图标更新不能遗漏 Web/PWA 入口

- 现象：桌面端品牌图标已经更新，但 LAN Web 的 favicon、Apple 主屏幕图标和 PWA manifest 仍指向旧的金色 A 图标。
- 原因：只盘点了 Electron Dock、安装包和官网资源，遗漏了 `src/web/public` 下独立维护的静态图标入口；同时自定义 Web 服务只放行了旧图标路径。
- 经验：品牌资源替换后要按消费入口逐项检查桌面 Dock、安装包、Windows/Linux、网页 favicon、Apple Touch Icon 和 PWA manifest，并验证每个 URL 都能从自定义 Web 服务正确取到。
- 处理：从当前品牌源图生成 180/192/512 PNG，更新 Web 页面和 manifest 引用，让旧 `icon.svg` 同步引用当前 Logo，并将新增图标路径加入 Web 服务的公开静态资源白名单。

## 2026-09-02：发布目录里的独立图标也要纳入品牌替换清单

- 现象：项目主品牌资源已经是新 Logo，但 Flatpak 发布目录里的 `flatpak/icon-512.png` 仍然是黑金波形图；Flatpak manifest 会直接把它安装到系统图标目录。
- 原因：Flatpak 使用独立的 512 像素文件，不会自动跟随 `brand/icon.png` 或 `brand/icon-linux.png` 更新。
- 经验：检查品牌资源时不能只看配置文件引用，还要打开每个被复制到安装包的独立二进制资源，确认视觉内容和尺寸都与当前品牌一致。
- 处理：用当前品牌源图重新生成 `flatpak/icon-512.png`，保留 Flatpak 所需的 512 像素尺寸约束。

## 2026-09-06：视频全屏时不能把控制条留在全屏目标之外

- 现象：全屏只作用于 `<video>` 时，播放器自己的控制条不在全屏 DOM 子树中；用户看到的全屏交互要么缺失，要么只能依赖浏览器默认媒体控制，底部收起还会因为 `max-height` 布局动画显得拖沓。
- 原因：全屏目标和控制层不在同一个渲染边界，且控制条收起同时动画布局尺寸、透明度和位移。
- 经验：需要自绘播放控制时，应让“视频画面、字幕和控制条”共享一个专用全屏舞台，不能把整个应用文档或侧栏一起全屏；全屏控制条隐藏只动画透明度和位移，时间轴轨道和拇指也应有明确的自绘视觉状态，并为 `prefers-reduced-motion` 提供静态回退。
- 处理：为播放器舞台增加全屏 ref，Fullscreen API 改为请求舞台，使用独立全屏样式把控制条固定在安全区内，并保留 `controls={false}` 让浏览器原生媒体控制不参与渲染。

## 2026-09-06：状态按钮不能让显示判定和点击判定各自维护一套真相

- 现象：全屏按钮显示的是退出图标，但点击路径却可能根据另一份 React 状态执行进入全屏；播放 / 暂停、静音 / 取消静音、开启 / 关闭随机播放也容易在事件延迟或状态切换后出现一次性反向图标、文案或动作。
- 原因：按钮的图标、标题、`aria-label` 和事件处理分别读取 `app.isFullscreen`、媒体元素属性、设置快照等不同来源，没有明确区分“当前状态”和“下一次点击动作”；浏览器 Fullscreen API 还可能在请求失败或外部元素全屏时异步改变状态。
- 经验：每个可逆控件必须先定义单一状态真相，再由纯函数同时生成当前表现和下一步动作；显示层与事件层必须调用同一个契约，不能在 JSX 和 handler 中重复写相反的三元判断。测试除了覆盖正常状态，还要覆盖结束媒体、请求失败、外部全屏和目标 ref 暂时为空。
- 处理：新增 `control-state.ts`，统一封装播放、静音和全屏动作映射；全屏显示直接读取 `document.fullscreenElement`，事件监听同时覆盖 `fullscreenchange` / `fullscreenerror`，随机播放补齐关闭 / 开启动作文案，窗口最大化与图片工作区入口同步修正图标和可逆语义，并增加纯函数与源码回归测试。

## 2026-09-07：全屏舞台铺满后还要同步放大视频元素

- 现象：播放器已经成功进入全屏，舞台尺寸也变成整个视口，但 640×360 的视频元素仍保持原始 640×360；用户看到的是中央小画面和大面积黑底，容易判断为“无法全屏”。
- 原因：普通播放预览为了保护横屏、竖屏和极端长图，使用了 `width: auto` / `height: auto` 配合 `max-width` / `max-height`。全屏请求目标改成舞台后，原先的 `.video-surface:fullscreen` 规则不会匹配，因为真正处于全屏状态的是父级舞台。
- 经验：全屏目标从媒体元素切换到自绘舞台时，必须同时检查舞台、媒体元素和控制层三个渲染边界；普通预览的自然尺寸约束不能直接继承到全屏。全屏应让视频元素占满舞台，再用 `object-fit: contain` 保留完整画面，比例不一致时只留下必要黑边；真实 Smoke 要断言视频 bounding box 与全屏舞台一致，不能只断言 `document.fullscreenElement` 和按钮状态。
- 处理：新增 `.stage:fullscreen .video-surface` 的全屏尺寸规则，扩展视频布局单测和真实 Electron Smoke，使用实际 640×360 文件验证全屏前后尺寸及退出状态。

## 2026-09-07：通用弹窗层级必须高于播放控制栏

- 现象：打开偏好设置时，底部播放控制栏仍浮在设置窗口上方，遮挡设置内容并可能抢占鼠标点击。
...
## 2026-09-07：普通播放预览不能沿用视频自然尺寸

- 现象：开发模式播放 360p 视频时，画面只按源视频尺寸居中显示，播放器舞台周围出现大面积黑边；全屏状态才会放大视频元素。
- 原因：普通 `.video-surface` 使用 `width: auto` / `height: auto`，虽然设置了最大尺寸约束，但不会主动占满舞台；全屏样式只覆盖全屏状态，无法修复普通预览。
- 经验：播放器的“铺满”应区分视频元素尺寸和视频内容裁剪方式：视频元素应占满可用舞台，内容继续使用 `object-fit: contain` 保持比例，不能用自然尺寸居中代替响应式布局。
- 处理：普通播放的视频元素改为 `width: 100%`、`height: 100%`，保留 `object-fit: contain`；扩展布局单测和真实播放 Smoke，断言非全屏视频 bounding box 与视频框一致。

## 2026-09-07：全屏播放控制栏不能覆盖视频内容

- 现象：全屏播放时底部控制栏悬浮在视频画面上方，遮挡人物、字幕或其他关键内容，尤其是在暂停状态下控制栏持续显示时更明显。
- 原因：全屏舞台使用绝对定位让视频框和控制栏同时覆盖整个视口；控制栏虽然支持自动隐藏，但显示期间没有为它保留独立布局空间。
- 经验：全屏播放器的控制层应优先采用“底部独立控制区 + 视频区域自适应”的布局，不能只依赖半透明背景或自动隐藏来缓解遮挡；字幕也应绑定视频区域底部，而不是绑定整个视口底部。
- 处理：将全屏舞台改为两行 Grid，视频框占第一行、控制栏占第二行；控制栏隐藏时折叠第二行，字幕固定在视频行底部，并在真实全屏 Smoke 中断言控制栏顶部不低于视频框底部。

## 2026-09-09：全屏 Grid 不能依赖子元素自动排版

- 现象：v0.6.10 中全屏控制栏在部分真实窗口状态下跑到顶部；源码 Smoke 通过，但用户安装包仍能复现。
- 原因：全屏舞台只给字幕和控制栏设置了 Grid 行，视频框没有绑定同一个明确的布局区域；不同字幕状态或运行时布局变化会触发 Grid 自动排版，导致三者的视觉顺序不稳定。此前还错误地把“源码修复已进入 tag”误判成“发布包行为一定已修复”。
- 经验：全屏媒体布局必须用命名 Grid 区域同时约束视频、字幕和控制栏，不能让任一直接子元素依赖自动放置；验证时要检查安装包实际行为和计算布局，不能只看版本号、tag 或单一源码断言。
- 处理：新增 `media` / `controls` 命名区域，视频框和字幕固定到 `media`，控制栏固定到 `controls`；Smoke 增加命名区域计算值断言。

## 2026-09-09：发布工作流步骤名变更必须同步契约测试

- 现象：完整测试发现 ARM64 Snap 发布工作流已经将 Smoke 步骤改名为 `Smoke test Snap CLI launcher`，源码契约测试仍断言旧名称，导致发布前测试失败。
- 原因：工作流可读性改动与对应源码测试没有在同一个变更中同步更新。
- 经验：发布工作流的步骤名称虽然不改变执行逻辑，但属于可审计契约；修改后必须立即搜索旧名称并重跑发布相关单测。
- 处理：同步 ARM64 Snap 契约测试断言，并将工作流步骤名漂移记录到本文件。
...
## 2026-09-07：半透明深色选中态不能继续使用深色文字

- 现象：素材库筛选的“全部”按钮选中后使用半透明金色底，但文字颜色为近黑色；在深色面板上对比度不足，用户很难确认当前筛选状态。
- 原因：`--accent-ink` 是给实心金色主按钮准备的反差文字色，不能直接套用到半透明强调底；组件局部选中背景与文字 token 的对比关系没有一起检查。
- 经验：选中态必须按实际背景计算对比度，实心强调按钮使用 `--accent-ink`，半透明强调底应使用 `--text-primary` 或高亮强调色，并通过边框 / 内嵌线补足状态识别；不能只看 token 名称判断文字一定可读。
- 处理：素材库筛选按钮改用正文高亮色、提高半透明强调底强度并增加底部强调线；保留实心主按钮的 `--accent-ink` 语义，补充样式源码回归检查和真实 Electron 截图验证。

## 2026-09-07：可视化播放头不能只做成穿透点击的装饰层

- 现象：剪辑时间线上的黄色播放头显示出可操作的视觉暗示，鼠标按住却无法拖动，事件穿透后会触发底层轨道定位，播放头甚至直接回到 00:00，交互反馈很差。
- 原因：播放头使用 `pointer-events: none` 仅作为展示层，没有独立的按下、移动、释放和指针捕获逻辑；底层片段 / 轨道默认处理器因此接管了用户本来针对播放头的操作。
- 经验：任何呈现为拖拽手柄的时间线元素都必须拥有独立命中区域、连续拖拽状态和 pointer capture，不能只通过鼠标样式暗示可操作；主视频轨和波形轨的同一播放头语义也应保持一致。
- 处理：将主轨播放头改为可访问的 slider button，拖拽期间连续吸附定位并在释放后结束捕获；波形播放头同步支持拖拽，保留键盘微调和原有轨道点击定位。

## 2026-09-07：时间轴轨道标签不能允许长名称自由换行

- 现象：空的画中画 / 分屏、图形、字幕轨道左侧标签字号显得过大，其中复合标签换成两行后进一步挤压时间轴，左侧栏视觉权重失衡。
- 原因：轨道标签虽然有基础字号，但没有统一的单行、截断和紧凑行高契约；同时可拖拽标签按钮上的 `font: inherit` 简写会重置前面设置的字号，导致按钮标签回到页面默认字号。
- 经验：时间轴轨道名属于导航元数据，不应抢占素材内容空间；应使用小字号、单行省略和 title / aria-label 保留完整语义，不能让长文本通过换行改变轨道布局。
- 处理：轨道标签统一为 8px、1.1 行高、单行省略，按钮只继承字体族、字号、字重和行高，不再用简写覆盖；完整名称继续通过现有悬停提示和无障碍标签提供，真实 Electron Smoke 增加计算样式断言。

## 2026-09-07：剪辑工作区不能为了紧凑而让全部正文降到微型字号

- 现象：剪辑面板顶部工具、素材库、脚本和时间轴的正文与提示普遍只有 8–11px，用户长时间查看时眼睛疲劳，信息层级也显得不正常。
- 原因：时间轴此前按“尽量塞下更多控件”的思路分别压缩了各个 CSS 文件中的字号，没有建立剪辑工作区的可读字号层级；轨道标签的单独修复又让其它内容的相对偏小更明显。
- 经验：密集型编辑器应先保证核心正文、操作按钮和状态信息达到稳定的可读下限，再通过换行、滚动和省略解决空间问题；不能把所有文本都压到 8–9px 后再依赖高 DPI 显示器补偿。
- 处理：建立剪辑工作区一档统一的可读性升级：标题 13px、主要工具 11px、正文 10px、时间轴标签 9–10px，保留单行省略和弹性换行，并由真实 Electron Smoke 读取计算字号验证。

## 2026-09-07：视频片段按钮不能截断时间轴点击定位

- 现象：点击波形轨可以把黄色播放头定位到点击位置，但点击铺满视频轨的素材片段没有定位效果，播放状态还可能回到 00:00，用户会认为视频轨不可操作。
- 原因：视频片段使用按钮承载选中和排序交互，并在 `onClick` 中无条件 `stopPropagation()`，导致父级视频轨的按位置 seek 处理器永远收不到点击事件；波形轨没有同样的子按钮拦截，所以两条轨道行为不一致。
- 经验：时间轴中的“可选择片段”和“可定位轨道”是两个正交语义，子元素可以处理选择，但不能无条件吞掉父级的定位点击；只有播放头、删除和裁切手柄等明确的独立操作才应阻止冒泡。
- 处理：视频片段点击保留选择逻辑并允许事件冒泡，由父级视频轨统一按点击位置 seek；片段拖拽仍在 pointer 事件阶段阻止冒泡，排序完成后继续抑制一次 click，避免拖拽误定位。
