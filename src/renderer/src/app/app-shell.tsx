...
export function AppShell(): React.ReactElement {
  ...
  return <div className="app-shell" data-theme={theme} onDragOver={(event) => event.preventDefault()} onDrop={onDrop}><AppHeader /><AppUpdateBanner copy={app.copy} state={app.appUpdateState} onCheck={() => { void app.checkForAppUpdate() }} onInstall={() => { void app.installAppUpdate() }} /><AsrModelBootstrapBanner copy={app.copy} state={app.asrModelBootstrapState} expectedSizeBytes={app.recommendedModelManifest?.expectedSizeBytes} onOpenAsrPanel={() => app.openPanelMode('asr')} /><div className="ai-workflow-slot"><AiWorkflowStatus /></div><div className="app-surface"><div className={`app-surface-pane ${app.viewMode === 'image' ? 'active' : ''}`} aria-hidden={app.viewMode !== 'image'}><ImageWorkspace /></div><div className={`app-surface-pane ${app.viewMode === 'video' ? 'active' : ''}`} aria-hidden={app.viewMode !== 'video'}><main className={`workspace ${app.isSidePanelVisible ? 'with-side-panel' : 'side-panel-collapsed'} ${sidePanelResize.isDragging ? 'is-resizing-side-panel' : ''}`} style={workspaceStyle}><PlayerStage /><AppSidePanel sidePanelResize={sidePanelResize} /></main></div></div><AppOverlays /></div>
}
