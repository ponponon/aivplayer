...
export function EditingFilterControl({ clip, title, brightnessLabel, contrastLabel, saturationLabel, resetLabel, onPreview, onChange }: EditingFilterControlProps): React.ReactElement | null {
  ...
  return <details className="editing-filter-control" onClick={(event) => event.stopPropagation()} data-testid="editing-filter-control">
    ...
    <div className="editing-filter-popover">
      {fields.map(([key, label]) => <label className="editing-filter-field" key={key}><span>{label}</span><input type="range" min="50" max="150" step="1" value={Math.round(draftValues[key] * 100)} onChange={(event) => updatePreview(key, Number(event.currentTarget.value) / 100)} onPointerDown={(event) => { interactionRef.current = true; event.currentTarget.setPointerCapture(event.pointerId) }} onPointerUp={commitPreview} onPointerCancel={commitPreview} onLostPointerCapture={commitPreview} onKeyDown={() => { interactionRef.current = true }} onKeyUp={commitPreview} onBlur={commitPreview} aria-label={label} /><output>{Math.round(draftValues[key] * 100)}%</output></label>)}
