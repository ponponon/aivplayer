...
async function runSmoke(): Promise<void> {
  ...
  try {
    ...
    await collectionOperationUndoButton.waitFor({ state: 'visible', timeout: 10_000 })
    const batchFlagUndoPersisted = await page.evaluate(({ favoriteTitle, archivedTitle, plainTitle }) => window.aiv.listVisionClipCollections().then((items) => {
      ...
      return favorite?.isFavorite === true && archived?.isFavorite === false && archived?.isArchived === true && plain?.isFavorite === false
    }), { favoriteTitle: titles[2], archivedTitle: titles[1], plainTitle: titles[0] })
    if (!batchFlagUndoPersisted) throw new Error('Batch favorite undo should restore all previous favorite states without changing archive state')
    await page.getByRole('button', { name: '收藏集合', exact: true }).click()
    await page.getByRole('button', { name: '取消收藏集合', exact: true }).waitFor({ timeout: 10_000 })
