...
export function registerVisionIpc(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.VISION_CLIP_COLLECTION_CONTENT_UPDATE, (_event, request: VisionClipCollectionContentUpdateRequest) => {
    const copy = getAppCopy(getCurrentLocale()).vision
    if (!request || typeof request.collectionId !== 'string') return { success: false, message: copy.collectionContentUpdateUnavailable, collection: null }
    try {
      const collection = getClipInboxStore().updateCollectionSelections(request.collectionId, request.selections)
      if (!collection) return { success: false, message: copy.collectionContentUpdateUnavailable, collection: null }
      return { success: true, message: copy.collectionContentUpdated(collection.title, collection.selections.length), collection }
    } catch (error) {
      return { success: false, message: error instanceof Error ? error.message : String(error), collection: null }
    }
  })
  ipcMain.handle(IPC_CHANNELS.VISION_CLIP_COLLECTION_BATCH_CONTENT_UPDATE, (_event, request: VisionClipCollectionBatchContentUpdateRequest) => {
    const copy = getAppCopy(getCurrentLocale()).vision
    if (!request || !Array.isArray(request.updates)) return { success: false, message: copy.collectionContentUpdateUnavailable, collections: [], skippedCount: 0 }
    try {
      const result = getClipInboxStore().updateCollectionsSelections(request.updates)
      return { success: true, message: copy.collectionContentBatchUpdated(result.collections.length, result.skippedCount), collections: result.collections, skippedCount: result.skippedCount }
    } catch (error) {
  ...
  ipcMain.handle(IPC_CHANNELS.VISION_CLIP_COLLECTION_TAGS_UPDATE, (_event, request: VisionClipCollectionTagUpdateRequest) => {
    const copy = getAppCopy(getCurrentLocale()).vision
    if (!request || typeof request.collectionId !== 'string') return { success: false, message: copy.collectionTagsUpdateUnavailable, collection: null }
    try {
      const collection = getClipInboxStore().updateCollectionTags(request.collectionId, request.tags)
      if (!collection) return { success: false, message: copy.collectionTagsUpdateUnavailable, collection: null }
      return { success: true, message: copy.collectionTagsUpdated(collection.title), collection }
    } catch (error) {
      return { success: false, message: error instanceof Error ? error.message : String(error), collection: null }
    }
  })
  ...
  ipcMain.handle(IPC_CHANNELS.VISION_CLIP_COLLECTION_OPERATION_BATCH_UNDO, (_event, operationIds: unknown) => {
    const copy = getAppCopy(getCurrentLocale()).vision
    try {
      const result = getClipInboxStore().undoCollectionOperations(operationIds as readonly unknown[])
  ...
  ipcMain.handle(IPC_CHANNELS.VISION_CLIP_COLLECTION_OPERATION_BATCH_REDO, (_event, operationIds: unknown) => {
    const copy = getAppCopy(getCurrentLocale()).vision
    try {
      const result = getClipInboxStore().redoCollectionOperations(operationIds as readonly unknown[])
