import { EDITING_PUNCH_IN_MAX_SCALE, EDITING_PUNCH_IN_MIN_SCALE, EDITING_PROJECT_SCHEMA_VERSION, EDITING_TRANSITION_MAX_DURATION, EDITING_TRANSITION_MIN_DURATION, EDITING_TREATMENT_SIZE_MAX, EDITING_TREATMENT_SIZE_MIN, type EditingCaptionPathHints, type EditingCaptionReloadResolution, type EditingCanvasPresetId, type EditingCaption, type EditingCaptionEffect, type EditingCaptionWord, type EditingCaptionPreferredPaths, type EditingCaptionSourceRevisions, type EditingClipFilter, type EditingClipTransition, type EditingClipTreatment, type EditingFrameId, type EditingGraphic, type EditingGraphicMotion, type EditingOverlayTrackKind, type EditingPersonMatte, type EditingProject, type EditingScriptSegment, type EditingSource, type EditingVideoBlock, type EditingVideoBlockMotion, type EditingVideoClip } from '../../shared/editing-types'
...
function parseVideoClip(value: unknown, sourceIds: Set<string>): EditingVideoClip | null {
  ...
  if (value.treatment !== undefined && value.treatment !== 'full' && value.treatment !== 'punch-in' && value.treatment !== 'corner-br' && value.treatment !== 'corner-tl' && value.treatment !== 'split-left' && value.treatment !== 'split-right') return null
  ...
  if (value.treatmentSize !== undefined && (typeof value.treatmentSize !== 'number' || !Number.isFinite(value.treatmentSize) || value.treatmentSize < EDITING_TREATMENT_SIZE_MIN || value.treatmentSize > EDITING_TREATMENT_SIZE_MAX)) return null
  ...
  const personMatte = value.personMatte === undefined ? undefined : isEditingPersonMatte(value.personMatte) ? value.personMatte as EditingPersonMatte : null
  ...
  const enterMotion = value.enterMotion as EditingGraphicMotion | undefined
  const exitMotion = value.exitMotion as EditingGraphicMotion | undefined
  const treatment = value.treatment as EditingClipTreatment | undefined
  return { id: value.id, sourceId: value.sourceId, sourceStartSeconds: value.sourceStartSeconds, sourceEndSeconds: value.sourceEndSeconds, ...(value.volume === undefined ? {} : { volume: value.volume }), ...(value.muted === undefined ? {} : { muted: value.muted }), ...(treatment === undefined ? {} : { treatment }), ...(value.treatmentScale === undefined ? {} : { treatmentScale: value.treatmentScale }), ...(value.treatmentAnchor === undefined ? {} : { treatmentAnchor: value.treatmentAnchor }), ...(value.treatmentSize === undefined ? {} : { treatmentSize: value.treatmentSize }), ...(filter === undefined ? {} : { filter: filter as EditingClipFilter }), ...(personMatte === undefined ? {} : { personMatte: personMatte as EditingPersonMatte }), ...(transitionIn === undefined ? {} : { transitionIn: transitionIn as EditingClipTransition }), ...(enterMotion === undefined ? {} : { enterMotion }), ...(exitMotion === undefined ? {} : { exitMotion }), ...(value.motionDurationSeconds === undefined ? {} : { motionDurationSeconds: value.motionDurationSeconds }) }
}
...
function parseCaptionSourceRevisions(value: unknown, sourceIds: Set<string>): EditingCaptionSourceRevisions | null {
  if (!isRecord(value)) return null
  const revisions: EditingCaptionSourceRevisions = {}
  for (const [sourceId, revision] of Object.entries(value)) {
    if (!sourceIds.has(sourceId) || !isRecord(revision)) return null
    if (!Object.prototype.hasOwnProperty.call(revision, 'source') || !Object.prototype.hasOwnProperty.call(revision, 'translation')) return null
    if (revision.source !== null && !isFiniteNonNegative(revision.source)) return null
    if (revision.translation !== null && !isFiniteNonNegative(revision.translation)) return null
    revisions[sourceId] = { source: revision.source as number | null, translation: revision.translation as number | null }
  }
  return revisions
}
...
function parseCaptionPathHints(value: unknown, sourceIds: Set<string>): EditingCaptionPathHints | null {
  if (!isRecord(value)) return null
  const hints: EditingCaptionPathHints = {}
  for (const [sourceId, sourcePaths] of Object.entries(value)) {
    if (!sourceIds.has(sourceId) || !isRecord(sourcePaths)) return null
    if (!Object.prototype.hasOwnProperty.call(sourcePaths, 'source') || !Object.prototype.hasOwnProperty.call(sourcePaths, 'translation')) return null
    if (sourcePaths.source !== null && !isRelativePathHint(sourcePaths.source)) return null
    if (sourcePaths.translation !== null && !isRelativePathHint(sourcePaths.translation)) return null
    hints[sourceId] = { source: sourcePaths.source as string | null, translation: sourcePaths.translation as string | null }
  }
  return hints
}
...
export function parseEditingProject(value: unknown): EditingProject {
  ...
  const captionSourcePathHints = value.captionSourcePathHints === undefined ? undefined : parseCaptionPathHints(value.captionSourcePathHints, sourceIds)
  if (captionSourcePathHints === null) throw new Error('Invalid editing project caption source path hints')
