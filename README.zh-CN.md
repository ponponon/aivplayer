...
这是旧版安装包没有把 LanceDB 的运行时依赖一起打包导致的启动问题，不是视频文件名、外置硬盘路径或 MP4 编码的问题；当前 `v0.5.3` 发布包已经包含修复。请直接下载对应 Release 的安装包，不要在应用包内手动安装 npm 依赖。

当前发布版本为 `v0.5.3`，请优先使用对应 Release 的安装包。
