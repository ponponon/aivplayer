...
describe('subtitle sidecar restoration contract', () => {
  it('keeps sidecar restoration scoped to the current media and cache fallback', () => {
    ...
    expect(ipc).toContain('INVALID_SUBTITLE_SIDECAR')
    expect(effects).toContain('INVALID_SUBTITLE_SIDECAR')
    expect(effects).toContain('model.setAsrNotice(sidecar)')
    expect(readSource('src/renderer/src/app/subtitle-tools-card.tsx')).toContain('data-testid="subtitle-asr-notice"')
