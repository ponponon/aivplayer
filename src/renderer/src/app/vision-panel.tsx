import { Archive, Check, CheckSquare, ChevronDown, ChevronRight, ChevronUp, Copy, Database, Download, FilePlus, ImageUp, Pencil, ScanSearch, Search, Square, Star, Tags, Trash2, Undo2, Upload, X } from 'lucide-react'
...
export function VisionPanel(): React.ReactElement {
  ...
  const toggleCollectionFlag = async (collection: VisionClipCollection, flag: 'isFavorite' | 'isArchived'): Promise<void> => {
    ...
    setError(null)
    try {
      const updated = flag === 'isFavorite'
        ? await updateCollection(collection, { isFavorite: enabled })
        : await updateCollection(collection, { isArchived: enabled })
      if (!updated) return
  ...
  const setSelectedCollectionFlag = async (flag: 'isFavorite' | 'isArchived', enabled: boolean): Promise<void> => {
    ...
    let updatedCount = 0
    try {
      for (const collection of selectedCollectionsForRename) {
        const updated = await window.aiv.saveVisionClipCollection({
          id: collection.id,
          title: collection.title,
          tags: collection.tags,
          sortMode: collection.sortMode,
          isFavorite: flag === 'isFavorite' ? enabled : collection.isFavorite,
          isArchived: flag === 'isArchived' ? enabled : collection.isArchived,
      ...
      const label = flag === 'isFavorite'
        ? (enabled ? app.copy.vision.collectionStatusFavorite : app.copy.vision.collectionStatusUnfavorite)
        : (enabled ? app.copy.vision.collectionStatusArchived : app.copy.vision.collectionStatusUnarchived)
      setCollectionTransferStatus(app.copy.vision.collectionsStatusUpdated(updatedCount, label))
    } catch (reason: unknown) {
      setError(reason instanceof Error ? reason.message : String(reason))
    } finally {
