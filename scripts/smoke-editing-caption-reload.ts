...
async function main(): Promise<void> {
  ...
  try {
    ...
    await page.waitForFunction(() => document.querySelectorAll('.editing-caption-reload-row').length === 1 && document.querySelector('.editing-caption-reload-row')?.textContent?.includes('原始译文 18') === true, null, { timeout: 10_000 })
    const remainingKeptTranslationRows = await page.locator('.editing-caption-reload-row').filter({ hasText: '原始译文 18' }).count()
    const translationRemovedRow = preview.locator('.editing-caption-reload-row').filter({ hasText: '原始译文 18' })
    const translationRemoveButton = translationRemovedRow.locator('[data-testid^="editing-caption-reload-remove-"]')
    const translationRemoveButtonCount = await translationRemoveButton.count()
    await translationRemoveButton.click()
    await page.waitForFunction(() => Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).some((item) => item.textContent?.includes('原始字幕 18') === true) && Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).every((item) => item.textContent?.includes('原始译文 18') !== true) && Array.from(document.querySelectorAll('[data-testid^="editing-script-row-"]')).some((row) => row.textContent?.replace(/\s+/g, '').includes('原始字幕18') === true && !row.classList.contains('is-deleted')), null, { timeout: 10_000 })
    const translationOnlySourceCaptionCount = await page.locator('[data-testid^="editing-caption-item-"]').filter({ hasText: '原始字幕 18' }).count()
    const translationOnlyTranslationCaptionCount = await page.locator('[data-testid^="editing-caption-item-"]').filter({ hasText: '原始译文 18' }).count()
    const translationOnlyScriptClass = await removedScriptRow.getAttribute('class')
    await page.waitForFunction(() => (document.querySelector('[data-testid="editing-caption-reload-search"]') as HTMLInputElement | null)?.value === '', null, { timeout: 10_000 })
    await search.fill('原始译文 18')
    await page.waitForFunction(() => document.querySelectorAll('.editing-caption-reload-row').length === 0, null, { timeout: 10_000 })
    const remainingTranslationRemovedRows = await page.locator('.editing-caption-reload-row').filter({ hasText: '原始译文 18' }).count()
    await page.locator('[data-testid="editing-undo"]').click()
    await page.waitForFunction(() => Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).some((item) => item.textContent?.includes('原始译文 18') === true), null, { timeout: 10_000 })
    const undoTranslationOnlyCaptionCount = await page.locator('[data-testid^="editing-caption-item-"]').filter({ hasText: '原始译文 18' }).count()
    await page.locator('[data-testid="editing-redo"]').click()
    await page.waitForFunction(() => Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).every((item) => item.textContent?.includes('原始译文 18') !== true), null, { timeout: 10_000 })
    const redoTranslationOnlyCaptionCount = await page.locator('[data-testid^="editing-caption-item-"]').filter({ hasText: '原始译文 18' }).count()
    await page.locator('[data-testid="editing-undo"]').click()
    await page.waitForFunction(() => Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).some((item) => item.textContent?.includes('原始译文 18') === true), null, { timeout: 10_000 })
    await search.fill('')
    await page.locator('[data-testid="editing-undo"]').click()
    await page.waitForFunction(() => Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).some((item) => item.textContent?.includes('原始字幕 18') === true) && Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).some((item) => item.textContent?.includes('原始译文 18') === true), null, { timeout: 10_000 })
    await search.fill('原始字幕 18')
    await page.waitForFunction(() => document.querySelectorAll('.editing-caption-reload-row').length === 1 && document.querySelector('.editing-caption-reload-row')?.textContent?.includes('原始字幕 18') === true, null, { timeout: 10_000 })
    ...
    await search.fill('原始译文 18')
    await page.waitForFunction(() => document.querySelectorAll('.editing-caption-reload-row').length === 1 && document.querySelector('.editing-caption-reload-row')?.textContent?.includes('原始译文 18') === true, null, { timeout: 10_000 })
    const translationKeepCurrentButton = preview.locator('.editing-caption-reload-row').filter({ hasText: '原始译文 18' }).locator('[data-testid^="editing-caption-reload-keep-current-"]')
    const translationKeepButtonCount = await translationKeepCurrentButton.count()
    await translationKeepCurrentButton.click()
    await page.waitForFunction(() => Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).some((item) => item.textContent?.includes('原始字幕 18') === true) && Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).some((item) => item.textContent?.includes('原始译文 18') === true), null, { timeout: 10_000 })
    await page.waitForFunction(() => (document.querySelector('[data-testid="editing-caption-reload-search"]') as HTMLInputElement | null)?.value === '', null, { timeout: 10_000 })
    await search.fill('原始字幕 18')
    await page.waitForFunction(() => document.querySelectorAll('.editing-caption-reload-row').length === 1 && document.querySelector('.editing-caption-reload-row')?.textContent?.includes('原始字幕 18') === true, null, { timeout: 10_000 })
    const sourceRemoveAfterTranslationKeep = preview.locator('.editing-caption-reload-row').filter({ hasText: '原始字幕 18' }).locator('[data-testid^="editing-caption-reload-remove-"]')
    const sourceRemoveAfterTranslationKeepButtonCount = await sourceRemoveAfterTranslationKeep.count()
    await sourceRemoveAfterTranslationKeep.click()
    await page.waitForFunction(() => {
      ...
      const conflict = document.querySelector('[data-testid="editing-caption-reload-conflict"]')
      const remainingRows = Array.from(document.querySelectorAll('[data-testid="editing-caption-reload-preview"] .editing-caption-reload-row'))
      return scriptRow?.classList.contains('is-deleted') === true && !sourceCaptionPresent && translationCaptionPresent && conflict !== null && remainingRows.every((row) => !row.textContent?.includes('原始字幕 18') && !row.textContent?.includes('原始译文 18'))
    }, null, { timeout: 10_000 })
    const sourceRemoveAfterTranslationKeepSourceCaptionCount = await page.locator('[data-testid^="editing-caption-item-"]').filter({ hasText: '原始字幕 18' }).count()
    const sourceRemoveAfterTranslationKeepTranslationCaptionCount = await page.locator('[data-testid^="editing-caption-item-"]').filter({ hasText: '原始译文 18' }).count()
    const sourceRemoveAfterTranslationKeepScriptClass = await removedScriptRow.getAttribute('class')
    const sourceRemoveAfterTranslationKeepConflictHasOtherChanges = await page.locator('[data-testid="editing-caption-reload-conflict"]').count() === 1 && await page.locator('[data-testid="editing-caption-reload-preview"] .editing-caption-reload-row').evaluateAll((rows) => rows.every((row) => !row.textContent?.includes('原始字幕 18') && !row.textContent?.includes('原始译文 18')))
    await page.locator('[data-testid="editing-undo"]').click()
    ...
    const undoSourceRemoveAfterTranslationKeepSourceCaptionCount = await page.locator('[data-testid^="editing-caption-item-"]').filter({ hasText: '原始字幕 18' }).count()
    const undoSourceRemoveAfterTranslationKeepTranslationCaptionCount = await page.locator('[data-testid^="editing-caption-item-"]').filter({ hasText: '原始译文 18' }).count()
    const undoSourceRemoveAfterTranslationKeepScriptClass = await removedScriptRow.getAttribute('class')
    await page.locator('[data-testid="editing-undo"]').click()
    await page.waitForFunction(() => Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).some((item) => item.textContent?.includes('原始字幕 18') === true) && Array.from(document.querySelectorAll('[data-testid^="editing-caption-item-"]')).some((item) => item.textContent?.includes('原始译文 18') === true) && Array.from(document.querySelectorAll('[data-testid^="editing-script-row-"]')).some((row) => row.textContent?.replace(/\s+/g, '').includes('原始字幕18') === true && !row.classList.contains('is-deleted')), null, { timeout: 10_000 })
    await search.fill('')
    await page.waitForFunction(() => (document.querySelector('[data-testid="editing-caption-reload-search"]') as HTMLInputElement | null)?.value === '', null, { timeout: 10_000 })
