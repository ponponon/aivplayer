...
describe('vision clip collection repair wiring', () => {
  it('keeps batch repair behind a preview, batch IPC, and one content history update', () => {
    ...
    const settings = readFileSync(join(projectRoot, 'src/desktop/ipc-settings.ts'), 'utf8')
    ...
    expect(channel).toContain("GET_MEDIA_CONTENT_HASH: 'media:get-content-hash'")
    ...
    expect(settings).toContain('IPC_CHANNELS.GET_MEDIA_CONTENT_HASH')
    ...
    expect(preload).toContain('getMediaContentHash')
    expect(preload).toContain('recursive = false')
    expect(repair).toContain('createVisionClipCollectionRepairPlan')
    expect(repair).toContain("basis = 'content-hash'")
    expect(panel).toContain('prepareSelectedCollectionRepair')
    expect(panel).toContain('openFolderPicker')
    expect(panel).toContain('listMediaFilesInDirectory(directoryPath, true)')
    expect(panel).toContain('getMediaContentHash')
  ...
  it('provides the batch repair copy in every supported locale', () => {
    for (const locale of ['zh-CN', 'en-US', 'ja-JP', 'ko-KR']) {
      ...
      expect(source).toContain('collectionRepairBatchMatchedContentHash')
      expect(source).toContain('collectionRepairBatchDirectoryAction')
      expect(source).toContain('savingCollection')
