...
export function EditingGraphicEditor({ graphic, title, textLabel, textPlaceholder, saveLabel, positionLabel, styleLabel, titleStyleLabel, labelStyleLabel, durationLabel, enterLabel, exitLabel, motionDurationLabel, motionLabels, assetSaveLabel, positionLabels, timelineDuration, onSaveAsset, onUpdate }: EditingGraphicEditorProps): React.ReactElement | null {
  ...
  return <details className="editing-graphic-editor" open={isOpen} onToggle={(event) => setIsOpen(event.currentTarget.open)} data-testid="editing-graphic-editor" onClick={(event) => event.stopPropagation()}>
    ...
    <form className="editing-graphic-popover" onSubmit={submit}>
      ...
      <label className="editing-graphic-field"><span>{enterLabel}</span><AppSelect value={enterMotion} onChange={(event) => setEnterMotion(event.currentTarget.value as EditingGraphicMotion)} data-testid="editing-graphic-edit-enter">{EDITING_GRAPHIC_MOTIONS.map((key) => <option key={key} value={key}>{motionLabels[key]}</option>)}</AppSelect></label>
      <label className="editing-graphic-field"><span>{exitLabel}</span><AppSelect value={exitMotion} onChange={(event) => setExitMotion(event.currentTarget.value as EditingGraphicMotion)} data-testid="editing-graphic-edit-exit">{EDITING_GRAPHIC_MOTIONS.map((key) => <option key={key} value={key}>{motionLabels[key]}</option>)}</AppSelect></label>
