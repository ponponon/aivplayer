...
function WindowControls(): React.ReactElement {
  ...
  return (
    <div className="window-controls">
      ...
      <button className="window-control" type="button" onClick={toggleMaximize} title={isMaximized ? app.copy.topbar.restoreWindow : app.copy.topbar.maximizeWindow} aria-label={isMaximized ? app.copy.topbar.restoreWindow : app.copy.topbar.maximizeWindow} aria-pressed={isMaximized}>
        {isMaximized ? <Minimize2 size={15} strokeWidth={1.8} /> : <Maximize2 size={15} strokeWidth={1.8} />}
      </button>
...
export function AppHeader(): React.ReactElement {
  ...
  const currentTheme = themePreference === 'system'
    ...
    : themePreference
  const ThemeIcon = themePreference === 'system' ? SunMoon : currentTheme === 'light' ? Moon : Sun
  const toggleTheme = (): void => {
    const nextTheme: Exclude<AppThemePreference, 'system'> = currentTheme === 'light' ? 'dark' : 'light'
    app.patchAppSettingsSection('ui', { theme: nextTheme })
  }
  ...
  return (
    <header className="titlebar">
      ...
      <nav className="top-actions" aria-label="Primary">
        ...
        <button className={`tool-button ${state.panelMode === 'asr' ? 'active' : ''}`} type="button" onClick={() => app.togglePanelMode('asr')} title={copy.topbar.toggleAsr} aria-label={copy.topbar.toggleAsr} aria-pressed={state.panelMode === 'asr'}><Sparkles size={17} /></button>
        <button className={`tool-button ${state.panelMode === 'batch' ? 'active' : ''}`} type="button" onClick={() => app.togglePanelMode('batch')} title={copy.topbar.toggleBatch} aria-label={copy.topbar.toggleBatch} aria-pressed={state.panelMode === 'batch'}><ListChecks size={17} /></button>
        <button className={`tool-button ${state.panelMode === 'summary' ? 'active' : ''}`} type="button" onClick={() => app.togglePanelMode('summary')} title={copy.topbar.toggleSummary} aria-label={copy.topbar.toggleSummary} aria-pressed={state.panelMode === 'summary'}><FileText size={17} /></button>
        <button className={`tool-button ${state.panelMode === 'vision' ? 'active' : ''}`} type="button" onClick={() => app.togglePanelMode('vision')} title={copy.topbar.toggleVision} aria-label={copy.topbar.toggleVision} aria-pressed={state.panelMode === 'vision'}><ScanSearch size={17} /></button>
        <button className={`tool-button ${state.panelMode === 'drama' ? 'active' : ''}`} type="button" onClick={() => app.togglePanelMode('drama')} title={copy.topbar.toggleDrama} aria-label={copy.topbar.toggleDrama} aria-pressed={state.panelMode === 'drama'}><Clapperboard size={17} /></button>
        <button className={`tool-button ${state.panelMode === 'info' ? 'active' : ''}`} type="button" onClick={() => app.togglePanelMode('info')} title={copy.topbar.toggleInfo} aria-label={copy.topbar.toggleInfo} aria-pressed={state.panelMode === 'info'}><Info size={17} /></button>
        <button className={`tool-button web-share-tool-button ${app.webShareStatus.running ? 'active' : ''}`} type="button" onClick={app.webShareStatus.running ? app.openWebShareDialog : () => void app.startWebShare()} title={webShareLabel} aria-label={webShareLabel} aria-haspopup={app.webShareStatus.running ? 'dialog' : undefined}><Share2 size={17} /></button>
