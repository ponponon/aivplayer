    ...
    <div className="vision-entity-catalog-list">
      {catalog.entries.map((entry) => {
        ...
        return <article className={`vision-entity-catalog-row ${entry.hidden ? 'is-hidden' : ''}`} key={entry.labelId}>
          ...
          <AppSelect className="vision-entity-catalog-merge" value={entry.mergedInto ?? ''} aria-label={copy.entityCatalogMergeLabel} onChange={(event) => void updateEntry(entry, { labelId: entry.labelId, mergedInto: event.target.value || null })}>
