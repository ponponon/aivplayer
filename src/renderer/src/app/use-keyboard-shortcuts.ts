...
export function useKeyboardShortcuts(model: AppModel, actions: KeyboardActions): void {
  ...
  const isEditing = (target: EventTarget | null): boolean => target instanceof HTMLInputElement || target instanceof HTMLTextAreaElement || target instanceof HTMLSelectElement || target instanceof HTMLElement && (target.isContentEditable || Boolean(target.closest('.app-select, .app-select-menu')))
    const onKeyDown = (event: KeyboardEvent): void => {
      if (model.isDownloadDialogOpen || model.isSettingsDialogOpen || model.isAboutDialogOpen || model.isSupportDialogOpen || model.isClipExportDialogOpen || model.isExportingClip) return
      if (event.key === 'Escape') {
  ...
  }, [model.state.currentFile?.path, model.state.playbackRate, model.appSettings.playback.seekStepSeconds, model.appSettings.playback.holdRightArrowSpeed, model.isDownloadDialogOpen, model.isSettingsDialogOpen, model.isAboutDialogOpen, model.isSupportDialogOpen, model.isClipExportDialogOpen, model.isExportingClip])
}
