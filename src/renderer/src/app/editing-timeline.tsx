    ...
    <section className="editing-timeline" data-testid="editing-timeline" aria-label={app.copy.editing.timelineLabel}>
      <div className="editing-toolbar">
        <div className="editing-toolbar-heading">
          ...
          {app.editingProjectStatus ? <div className={`editing-project-status ${app.editingProjectStatus.success ? 'is-success' : 'is-error'}`} role="status"><span className="editing-project-status-message" data-testid="editing-project-status-message">{app.editingProjectStatus.message}</span>{app.editingProjectStatus.details?.groups.length ? <details className="editing-project-status-details" data-testid="editing-project-status-details"><summary>{app.editingProjectStatus.details.label}</summary><div className="editing-project-status-details-groups">{app.editingProjectStatus.details.groups.map((group) => <section className="editing-project-status-details-group" data-testid={`editing-project-status-details-group-${group.id}`} key={group.id}><strong>{group.label}</strong><div className="editing-project-status-details-list">{group.items.map((item, index) => <span key={`${item}-${index}`}>{item}</span>)}</div></section>)}</div></details> : null}</div> : null}
        </div>
