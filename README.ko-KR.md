...
- 디렉터리 재귀 스캔, 증분 인덱싱, 백그라운드 인덱스 큐, 재생 목록 자동 스캔, 인덱스 진행률과 단계별 소요 시간 표시를 지원합니다.
...
고정된 프로젝트를 로컬 stdio MCP 방식으로 Agent에 제공할 수 있습니다. 서비스는 `inspect`, `captions`, `propose delete-script` 세 가지 읽기 전용 도구만 노출하며 네트워크 포트를 열지 않고 Proposal 적용, 파일 쓰기, 미디어 삭제 또는 셸 실행을 할 수 없습니다.
...
Windows NSIS, macOS `.pkg`와 Linux `.deb`는 `aivcli` 런처를 설치하고 시스템 명령 경로에 추가합니다. macOS `.dmg` / `.zip`과 Linux `.AppImage`는 포터블 형식이라 PATH를 자동으로 수정하지 않습니다. 포터블 형식에서는 앱의 `--cli` 모드를 직접 실행하거나 직접 명령줄 런처를 만들 수 있습니다.
...
이 문제는 이전 설치 파일이 LanceDB 런타임 의존성을 함께 패키징하지 않아 발생한 시작 오류입니다. 동영상 파일명, 외장 드라이브 경로 또는 MP4 인코딩이 원인이 아닙니다. 현재 `v0.5.0` 릴리스에는 수정 사항이 포함되어 있습니다. 해당 Release의 설치 파일을 다운로드하고 앱 번들 안에 npm 의존성을 직접 설치하지 마세요.

현재 릴리스는 `v0.5.0`이므로 해당 Release의 설치 파일을 우선 사용하세요.
