  export const enUS = {
    ...
    editing: {
      subtitleQaTitle: 'Subtitle QA', subtitleQaIssueCount: (count: number) => count === 0 ? 'Clear' : `${count} issue${count === 1 ? '' : 's'}`, subtitleQaClear: 'No obvious subtitle issues found', subtitleQaJump: 'Jump', subtitleQaKindLabels: { empty: 'Empty cue', overlap: 'Overlapping time', 'too-short': 'Too short', 'too-long': 'Too long', 'high-cps': 'High reading speed', 'wide-line': 'Line too wide', punctuation: 'Punctuation', recognition: 'Possible recognition error' }, subtitleQaMessage: (kind: string, value: number) => kind === 'overlap' ? `${value.toFixed(2)}s overlap` : kind === 'high-cps' ? `${value.toFixed(1)} chars/s` : kind === 'wide-line' ? `about ${value.toFixed(1)} em` : kind === 'too-short' || kind === 'too-long' ? `${value.toFixed(2)}s duration` : 'Review the subtitle text', waveformTrack: 'Waveform', waveformUnavailable: 'No audio waveform',
