...
describe('vision duplicate media wiring', () => {
  it('keeps the scan contract connected across IPC, preload, panel, and source card', () => {
    ...
    expect(ipc).toContain('VISION_DUPLICATE_MEDIA_CANCEL')
    expect(preload).toContain('scanVisionDuplicateMedia')
    expect(preload).toContain('cancelVisionDuplicateMedia')
    ...
    expect(sources).toContain('data-testid="vision-duplicate-cancel"')
    expect(sources).toContain('data-testid="vision-duplicate-export"')
    expect(sources).toContain('onExportDuplicates')
    expect(panel).toContain('serializeVisionDuplicateMediaAudit')
