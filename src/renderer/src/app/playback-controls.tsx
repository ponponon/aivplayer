...
import { Bookmark, BookmarkPlus, Fullscreen, Minimize2, Pause, Play, Repeat, Repeat1, Scissors, Shuffle, SkipBack, SkipForward, Square, Volume2, VolumeX, X } from 'lucide-react'
...
function PlaybackPrimaryControls({ activeStructureSegment }: { activeStructureSegment: MediaStructureSegment | null }): React.ReactElement {
  ...
  const transportAction = getTransportAction(state.isPlaying)
  const muteAction = getMuteAction(state.muted)
  const transportLabel = transportAction === 'pause' ? copy.controls.pause : copy.controls.play
  const muteLabel = muteAction === 'unmute' ? copy.controls.unmute : copy.controls.mute
  ...
  return <div className="controls-primary"><div className="control-group transport-group"><button className="round-button" type="button" onClick={() => app.playAdjacent(-1)} title={copy.controls.previous} aria-label={copy.controls.previous}><SkipBack size={16} /></button><button className="round-button primary" type="button" data-testid="playback-toggle" onClick={app.togglePlay} title={`${transportLabel} (Space)`} aria-label={transportLabel} aria-keyshortcuts="Space">{transportAction === 'pause' ? <Pause size={18} /> : <Play size={18} />}</button><button className="round-button" type="button" onClick={() => app.playAdjacent(1)} title={copy.controls.next} aria-label={copy.controls.next}><SkipForward size={16} /></button></div><button className="round-button stop-button" type="button" onClick={app.stopPlayback} title={`${copy.controls.stopAndReset} (S)`} aria-label={copy.controls.stopAndReset} aria-keyshortcuts="S"><Square size={14} fill="currentColor" stroke="none" /><span>{copy.controls.stop}</span></button>{activeStructureSegment && structureLabel ? <button className="round-button structure-skip-button" type="button" data-testid="playback-structure-skip" onClick={skipStructure} title={copy.controls.skipStructureAt(structureLabel, formatTime(activeStructureSegment.endSeconds))} aria-label={copy.controls.skipStructureAt(structureLabel, formatTime(activeStructureSegment.endSeconds))}><SkipForward size={14} /><span>{copy.controls.skipStructureAt(structureLabel, formatTime(activeStructureSegment.endSeconds))}</span></button> : null}<div className="control-group volume-group"><button className="round-button" type="button" onClick={app.toggleMute} title={muteLabel} aria-label={muteLabel} aria-pressed={state.muted}>{state.muted ? <VolumeX size={16} /> : <Volume2 size={16} />}</button><input className="volume" type="range" min="0" max="1" step="0.01" value={state.muted ? 0 : state.volume} onChange={(event) => { const volume = Number(event.currentTarget.value); const muted = volume === 0; if (app.videoRef.current) { app.videoRef.current.volume = volume; app.videoRef.current.muted = muted }; app.setState((current) => ({ ...current, volume, muted })); app.syncPlaybackMemory(volume, muted, state.playbackRate) }} aria-label={copy.controls.volume} /></div></div>
}
...
function PlaybackModeControls(): React.ReactElement {
  ...
  const shuffleEnabled = app.appSettings.playback.order === 'shuffle'
  const shuffleLabel = getShuffleAction(shuffleEnabled) === 'disable' ? copy.controls.shuffleOff : copy.controls.shuffleEnable
  return <div className="control-group playback-mode-group"><button className={`round-button mode-button ${app.appSettings.playback.repeatMode !== 'none' ? 'active' : ''}`} type="button" onClick={app.cycleRepeatMode} title={`${copy.controls.repeat}: ${repeatTitle}`} aria-label={`${copy.controls.repeat}: ${repeatTitle}`} aria-pressed={app.appSettings.playback.repeatMode !== 'none'}>{app.appSettings.playback.repeatMode === 'current' ? <Repeat1 size={16} /> : <Repeat size={16} />}</button><button className={`round-button mode-button ${shuffleEnabled ? 'active' : ''}`} type="button" onClick={app.togglePlaybackOrder} title={shuffleLabel} aria-label={shuffleLabel} aria-pressed={shuffleEnabled}><Shuffle size={16} /></button><button className="round-button mode-button" type="button" onClick={app.togglePlaybackEndAction} title={endActionTitle} aria-label={endActionTitle} aria-pressed={app.appSettings.playback.endAction === 'next'}>{app.appSettings.playback.endAction === 'next' ? <SkipForward size={16} /> : <Square size={13} fill="currentColor" stroke="none" />}</button></div>
}
...
function PlaybackSegmentControl(): React.ReactElement {
  ...
  return <details className="segment-control">
    ...
    <div className="segment-popover">
      ...
      <label className="segment-color-field"><span>{copy.controls.segmentColor}</span><AppSelect value={color} onChange={(event) => setColor(event.currentTarget.value as PlaybackSegmentColor)} aria-label={copy.controls.segmentColor}>{(['accent', 'cyan', 'violet', 'amber'] as const).map((option) => <option key={option} value={option}>{copy.controls.segmentColorNames[option]}</option>)}</AppSelect></label>
...
function PlaybackSecondaryControls(): React.ReactElement {
  const app = useAppContext()
  const { state, copy } = app
  const fullscreenState = getFullscreenControlState(document.fullscreenElement, app.fullscreenRef.current)
  return <div className="control-group secondary-group"><div className="speed-control"><AppSelect className="speed-select" value={state.playbackRate} onChange={(event) => { const playbackRate = Number(event.currentTarget.value); if (app.videoRef.current) app.videoRef.current.playbackRate = playbackRate; app.setState((current) => ({ ...current, playbackRate })); app.syncPlaybackMemory(state.volume, state.muted, playbackRate) }} aria-label={copy.controls.playbackSpeed}>{[0.5, 0.75, 1, 1.25, 1.5, 2].map((speed) => <option key={speed} value={speed}>{speed}x</option>)}</AppSelect></div><PlaybackModeControls /><button className="round-button" type="button" onClick={() => app.createPlaybackBookmark(copy.controls.bookmarkAt(formatTime(state.currentTime)))} title={copy.controls.addBookmark} aria-label={copy.controls.addBookmark}><BookmarkPlus size={16} /></button><PlaybackSegmentControl /><button className="round-button" type="button" onClick={() => void app.toggleFullscreen()} title={fullscreenState.isActive ? copy.controls.exitFullscreen : copy.controls.fullscreen} aria-label={fullscreenState.isActive ? copy.controls.exitFullscreen : copy.controls.fullscreen} aria-pressed={fullscreenState.isActive} aria-keyshortcuts="F">{fullscreenState.isActive ? <Minimize2 size={16} /> : <Fullscreen size={16} />}</button></div>
}
