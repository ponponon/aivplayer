...
import { EditingProposalConfirmDialog } from './editing-proposal-confirm-dialog'
...
function EditingProjectStatusView({ projectId, status, resetLabel, resetAllLabel, resetAllConfirm }: { projectId: string; status: EditingProjectStatus | null; resetLabel: string; resetAllLabel: string; resetAllConfirm: string }): React.ReactElement | null {
  ...
  const resetDetailsPreferences = (): void => {
    const storage = getEditingUiPreferenceStorage()
    if (storage) resetEditingUiProjectPreferences(storage, projectId)
    setDetailsOpen(false)
    setOpenGroups({})
  }
...
export function EditingTimeline(): React.ReactElement | null {
  ...
  const editingTransportAction = getTransportAction(app.state.isPlaying)
  ...
  const subtitleCandidateCopy = getEditingSubtitleCandidateCopy(app.appSettings.ui.locale)
  ...
  const [deleteProposal, setDeleteProposal] = useState<EditingProposal | null>(null)
  ...
  const proposeScriptDeletion = (segmentId: string): void => {
    setSelectedScriptSegmentId(segmentId)
    const proposal = app.createDeleteEditingScriptProposal([segmentId])
    if (proposal) setDeleteProposal(proposal)
  }
  const proposeScriptDeletions = (segmentIds: readonly string[]): void => {
    const firstSegmentId = segmentIds[0]
    if (firstSegmentId) setSelectedScriptSegmentId(firstSegmentId)
    const proposal = app.createDeleteEditingScriptProposal(segmentIds)
    if (proposal) setDeleteProposal(proposal)
  }
  const applyScriptDeletionProposal = (): void => {
    if (!deleteProposal) return
    app.applyEditingScriptProposal(deleteProposal)
    ...
    <section className="editing-timeline" data-testid="editing-timeline" aria-label={app.copy.editing.timelineLabel}>
      ...
      <div className="editing-action-row">
        <div className="editing-transport">
          <button className="editing-primary-button" type="button" onClick={() => void app.toggleEditingPlay()} title={editingTransportAction === 'pause' ? app.copy.controls.pause : app.copy.controls.play} aria-label={editingTransportAction === 'pause' ? app.copy.controls.pause : app.copy.controls.play} aria-pressed={editingTransportAction === 'pause'} data-testid="editing-play">{editingTransportAction === 'pause' ? <Pause size={16} /> : <Play size={16} />}</button>
        ...
        <EditingAudioControl clip={selectedClip} volumeLabel={app.copy.controls.volume} muteLabel={app.copy.controls.mute} unmuteLabel={app.copy.controls.unmute} onVolumeChange={(volume) => selectedClip && app.setEditingClipVolume(selectedClip.id, volume)} onToggleMute={() => selectedClip && app.toggleEditingClipMute(selectedClip.id)} />
