...
async function main(): Promise<void> {
  ...
  const scriptSentence = '第一句脚本嗯这是一个用于验证播放器自动分行、逐词高亮以及长字幕导出一致性的测试句子，应该被拆分成多个连续显示页面。'
  ...
  const fillerIndex = scriptSentence.indexOf('嗯')
  const bodySplit = fillerIndex + 1 + Math.ceil((scriptSentence.length - fillerIndex - 1) / 2)
  await writeFile(join(smokeDirectory, 'script-smoke.json'), JSON.stringify({ transcription: [{ timestamps: { from: '00:00:00,000', to: '00:00:02,000' }, text: scriptSentence, tokens: [{ text: scriptSentence.slice(0, fillerIndex), timestamps: { from: '00:00:00,000', to: '00:00:00,500' } }, { text: '嗯', timestamps: { from: '00:00:00,500', to: '00:00:00,700' } }, { text: scriptSentence.slice(fillerIndex + 1, bodySplit), timestamps: { from: '00:00:00,700', to: '00:00:01,350' } }, { text: scriptSentence.slice(bodySplit), timestamps: { from: '00:00:01,350', to: '00:00:02,000' } }] }] }))
  ...
  try {
    ...
    const personMatteControl = page.locator('[data-testid="editing-person-matte-control"]')
    await personMatteControl.waitFor({ timeout: 10_000 })
    await page.waitForFunction(() => document.querySelector('[data-testid="editing-person-matte-control"] strong')?.textContent !== '…', null, { timeout: 10_000 })
    const personMatteStatus = await personMatteControl.locator('strong').textContent()
    ...
    const firstScriptWord = page.locator('.editing-script-row').first().locator('.editing-script-word').first()
    await firstScriptWord.click()
    await page.locator('[data-testid^="editing-script-word-replace-input-"]').waitFor({ timeout: 10_000 })
    const wordPopoverScreenshotPath = join(smokeHomeDirectory, 'aivplayer-smoke-editing-word-popover.png')
    await page.screenshot({ path: wordPopoverScreenshotPath, fullPage: false })
    const replacementInput = page.locator('[data-testid^="editing-script-word-replace-input-"]').first()
    await replacementInput.fill('替换词')
    await page.locator('.editing-script-action[data-testid^="editing-script-word-replace-"]').first().click()
    ...
    const scriptTextAfterWordReplace = await page.locator('.editing-script-text').first().textContent()
    await page.locator('[data-testid="editing-undo"]').click()
    await page.waitForFunction((expected) => document.querySelector('.editing-script-text')?.textContent === expected, scriptTextBeforeWordDelete)
    const fillerDeleteAvailable = await page.locator('[data-testid="editing-script-filler-delete"]').count() > 0
    if (fillerDeleteAvailable) {
      await page.locator('[data-testid="editing-script-filler-delete"]').click()
      await page.waitForFunction((expected) => document.querySelector('.editing-script-text')?.textContent !== expected, scriptTextBeforeWordDelete)
      await page.locator('[data-testid="editing-undo"]').click()
      await page.waitForFunction((expected) => document.querySelector('.editing-script-text')?.textContent === expected, scriptTextBeforeWordDelete)
    }
    const firstRowWords = page.locator('.editing-script-row').first().locator('.editing-script-word')
    await firstRowWords.nth(0).click({ modifiers: ['Shift'] })
    await firstRowWords.nth(1).click({ modifiers: ['Shift'] })
    await page.locator('[data-testid="editing-script-delete-selected"]').waitFor({ timeout: 10_000 })
    const scriptTextBeforeBatchDelete = await page.locator('.editing-script-text').first().textContent()
    await page.locator('[data-testid="editing-script-delete-selected"]').click()
    await page.waitForFunction((expected) => document.querySelector('.editing-script-text')?.textContent !== expected, scriptTextBeforeBatchDelete)
    const scriptTextAfterBatchDelete = await page.locator('.editing-script-text').first().textContent()
    await page.locator('[data-testid="editing-undo"]').click()
    await page.waitForFunction((expected) => document.querySelector('.editing-script-text')?.textContent === expected, scriptTextBeforeWordDelete)
    await firstScriptWord.click()
    const firstWordDeleteAction = page.locator('[data-testid^="editing-script-word-delete-"]').first()
    await firstWordDeleteAction.waitFor({ timeout: 10_000 })
    await firstWordDeleteAction.click()
    await page.waitForFunction((expected) => document.querySelector('.editing-script-text')?.textContent !== expected, scriptTextBeforeWordDelete)
    ...
    await page.locator('[data-testid="editing-treatment-corner-br"]').click()
    await page.waitForFunction(() => { const video = document.querySelector('video.video-surface') as HTMLVideoElement | null; return video?.getAttribute('data-framing-treatment') === 'corner-br' && video.style.transform.includes('translate(') && video.style.transform.includes('scale(0.34)') })
    const compactTreatmentPreview = await page.evaluate(() => ({ treatment: document.querySelector('video.video-surface')?.getAttribute('data-framing-treatment') ?? '', transform: (document.querySelector('video.video-surface') as HTMLVideoElement | null)?.style.transform ?? '', origin: (document.querySelector('video.video-surface') as HTMLVideoElement | null)?.style.transformOrigin ?? '' }))
    await page.locator('[data-testid="editing-canvas-control"] .editing-canvas-summary').click()
    await page.locator('[data-testid="editing-canvas-landscape"]').click()
    await page.waitForFunction(() => document.querySelector('[data-testid="editing-treatment-control"]')?.getAttribute('data-framing-orientation') === 'landscape')
    const orientationTreatmentState = await page.evaluate(() => ({ hint: document.querySelector('[data-testid="editing-treatment-orientation-hint"]')?.textContent ?? '', cornerAllowed: !(document.querySelector('[data-testid="editing-treatment-corner-br"]') as HTMLButtonElement | null)?.disabled, splitAllowed: !(document.querySelector('[data-testid="editing-treatment-split-left"]') as HTMLButtonElement | null)?.disabled }))
    if (orientationTreatmentState.cornerAllowed || !orientationTreatmentState.splitAllowed) throw new Error(`Orientation framing rule failed: ${JSON.stringify(orientationTreatmentState)}`)
    await page.locator('[data-testid="editing-framing-preset-control"] .editing-framing-preset-summary').click()
    const orientationPresetState = await page.evaluate(() => ({ cornerAllowed: !(document.querySelector('[data-testid="editing-framing-preset-corner-br"]') as HTMLButtonElement | null)?.disabled, splitAllowed: !(document.querySelector('[data-testid="editing-framing-preset-split-left"]') as HTMLButtonElement | null)?.disabled }))
    if (orientationPresetState.cornerAllowed || !orientationPresetState.splitAllowed) throw new Error(`Orientation framing preset rule failed: ${JSON.stringify(orientationPresetState)}`)
    const framingPresetScreenshotPath = join(smokeHomeDirectory, 'aivplayer-smoke-editing-framing-presets.png')
    await page.screenshot({ path: framingPresetScreenshotPath, fullPage: false })
    await page.locator('[data-testid="editing-framing-preset-split-right"]').click()
    await page.waitForFunction(() => document.querySelector('video.video-surface')?.getAttribute('data-framing-treatment') === 'split-right')
    const framingPresetPreview = await page.evaluate(() => ({ treatment: document.querySelector('video.video-surface')?.getAttribute('data-framing-treatment') ?? '', controlOrientation: document.querySelector('[data-testid="editing-framing-preset-control"]')?.getAttribute('data-framing-orientation') ?? '' }))
    await page.locator('[data-testid="editing-treatment-split-left"]').click()
    await page.waitForFunction(() => { const video = document.querySelector('video.video-surface') as HTMLVideoElement | null; return video?.getAttribute('data-framing-treatment') === 'split-left' && video.classList.contains('is-framing-split-left') && !video.classList.contains('is-split-left') })
    const mainSplitTreatmentPreview = await page.evaluate(() => ({ treatment: document.querySelector('video.video-surface')?.getAttribute('data-framing-treatment') ?? '', className: document.querySelector('video.video-surface')?.className ?? '', transform: (document.querySelector('video.video-surface') as HTMLVideoElement | null)?.style.transform ?? '' }))
    await page.locator('[data-testid="editing-canvas-control"] .editing-canvas-summary').click()
    await page.locator('[data-testid="editing-canvas-portrait"]').click()
    await page.waitForFunction(() => document.querySelector('.stage')?.getAttribute('data-editing-canvas') === 'portrait')
    await page.locator('[data-testid="editing-treatment-punch-in"]').click()
    await page.locator('[data-testid="editing-treatment-control"] input[type="range"]').fill('1.6')
    await page.locator('[data-testid="editing-treatment-anchor-left"]').click()
    await page.waitForFunction(() => { const video = document.querySelector('video.video-surface') as HTMLVideoElement | null; return video?.classList.contains('is-punch-in') === true && video.style.transform === 'scale(1.6)' })
    await page.locator('[data-testid="editing-filter-control"] .editing-filter-summary').click()
    const filterSlider = page.locator('[data-testid="editing-filter-control"] input').first()
    await filterSlider.fill('120')
    ...
    await filterSlider.evaluate((element) => (element as HTMLInputElement).blur())
    await page.waitForFunction(() => (document.querySelector('video.video-surface') as HTMLVideoElement | null)?.style.filter === 'brightness(1.2) contrast(1) saturate(1)')
    const filterSliderBox = await filterSlider.boundingBox()
    if (!filterSliderBox) throw new Error('Filter slider was not measurable')
    await page.mouse.move(filterSliderBox.x + filterSliderBox.width * 0.7, filterSliderBox.y + filterSliderBox.height / 2)
    await page.mouse.down()
    await page.mouse.move(filterSliderBox.x + filterSliderBox.width * 0.9, filterSliderBox.y + filterSliderBox.height / 2)
    await page.mouse.move(filterSliderBox.x + filterSliderBox.width * 0.8, filterSliderBox.y + filterSliderBox.height / 2)
    await page.mouse.up()
    await page.waitForFunction(() => {
      const filter = (document.querySelector('video.video-surface') as HTMLVideoElement | null)?.style.filter ?? ''
      return filter.startsWith('brightness(') && filter !== 'brightness(1.2) contrast(1) saturate(1)'
    })
    const filterAfterSliderDrag = await page.evaluate(() => (document.querySelector('video.video-surface') as HTMLVideoElement | null)?.style.filter ?? '')
    await page.locator('[data-testid="editing-filter-control"] .editing-filter-summary').click()
    await page.locator('[data-testid="editing-undo"]').click()
    await page.waitForTimeout(250)
    const filterAfterOneUndo = await page.evaluate(() => (document.querySelector('video.video-surface') as HTMLVideoElement | null)?.style.filter ?? '')
    const undoDisabledAfterSlider = await page.locator('[data-testid="editing-undo"]').isDisabled()
    console.log(`Slider undo probe: ${JSON.stringify({ filterAfterSliderDrag, filterAfterOneUndo, undoDisabledAfterSlider })}`)
    await page.locator('[data-testid="editing-filter-control"] .editing-filter-summary').click()
    ...
    await page.locator('[data-testid="editing-treatment-control"]').waitFor({ timeout: 10_000 })
    await page.locator('[data-testid="editing-treatment-full"]').click()
    await page.locator('[data-testid^="editing-framing-marker-"]').first().waitFor({ timeout: 10_000 })
    const framingMarkerCount = await page.locator('[data-testid^="editing-framing-marker-"]').count()
    const framingMarkerLabel = await page.locator('[data-testid^="editing-framing-marker-"]').first().getAttribute('aria-label')
    await page.locator('[data-testid^="editing-framing-marker-"]').first().click()
    const selectedClipCountAfterFramingMarker = await page.locator('.editing-clip.is-selected').count()
    ...
    const cornerOutputPath = join(smokeHomeDirectory, 'aivplayer-smoke-corner-framing.mp4')
    const cornerExportResult = await page.evaluate(async ({ sourcePath, targetPath }) => window.aiv.exportMediaTimeline({ mediaPath: sourcePath, clips: [{ mediaPath: sourcePath, startSeconds: 0, endSeconds: 2, treatment: 'corner-br', treatmentSize: 35 }], mode: 'video', targetWidth: 1080, targetHeight: 1920, fitMode: 'cover', outputVideoPath: targetPath }), { sourcePath: mediaPath, targetPath: cornerOutputPath })
    const cornerOutputStats = cornerExportResult.success ? await stat(cornerOutputPath).catch(() => null) : null
    const mainSplitOutputPath = join(smokeHomeDirectory, 'aivplayer-smoke-main-split-framing.mp4')
    const mainSplitExportResult = await page.evaluate(async ({ sourcePath, targetPath }) => window.aiv.exportMediaTimeline({ mediaPath: sourcePath, clips: [{ mediaPath: sourcePath, startSeconds: 0, endSeconds: 2, treatment: 'split-left', treatmentSize: 50 }], mode: 'video', targetWidth: 1920, targetHeight: 1080, fitMode: 'cover', outputVideoPath: targetPath }), { sourcePath: mediaPath, targetPath: mainSplitOutputPath })
    const mainSplitOutputStats = mainSplitExportResult.success ? await stat(mainSplitOutputPath).catch(() => null) : null
    const framingTransitionOutputPath = join(smokeHomeDirectory, 'aivplayer-smoke-framing-transition.mp4')
    const framingTransitionExportResult = await page.evaluate(async ({ sourcePath, targetPath }) => window.aiv.exportMediaTimeline({ mediaPath: sourcePath, clips: [{ mediaPath: sourcePath, startSeconds: 0, endSeconds: 2 }, { mediaPath: sourcePath, startSeconds: 2, endSeconds: 4, treatment: 'punch-in', treatmentScale: 1.6, treatmentAnchor: 'left' }], mode: 'video', outputVideoPath: targetPath }), { sourcePath: mediaPath, targetPath: framingTransitionOutputPath })
    const framingTransitionOutputStats = framingTransitionExportResult.success ? await stat(framingTransitionOutputPath).catch(() => null) : null
    ...
    await page.waitForFunction(({ clipId, sourceId }) => {
      const stored = JSON.parse(localStorage.getItem('aivplayer.editing-projects.v1') ?? '{}') as Record<string, { videoClips?: Array<{ id: string; sourceId: string }> }>
      return Object.values(stored).some((project) => project.videoClips?.some((clip) => clip.id === clipId && clip.sourceId === sourceId) === true)
    }, { clipId: replacementTarget.clipId, sourceId: replacementTarget.replacementSourceId }, { timeout: 10_000 })
    ...
    await page.locator('.editing-export-confirm-cancel').evaluate((element) => (element as HTMLButtonElement).click())
    ...
    console.log(`Script word actions: ${JSON.stringify({ wordCount: scriptWordCount, before: scriptTextBeforeWordDelete, replaced: scriptTextAfterWordReplace, batch: scriptTextAfterBatchDelete, after: scriptTextAfterWordDelete, fillerDeleteAvailable, restored: wordDeleteRestored })}`)
    console.log(`Script word popover screenshot: ${wordPopoverScreenshotPath}`)
    ...
    console.log(`Person matte model setup: ${JSON.stringify({ status: personMatteStatus ?? '', downloadAvailable: personMatteDownloadAvailable })}`)
    ...
    console.log(`Framing markers: ${JSON.stringify({ count: framingMarkerCount, label: framingMarkerLabel ?? '', selectedClipCount: selectedClipCountAfterFramingMarker })}`)
    ...
    console.log(`Compact framing preview: ${JSON.stringify(compactTreatmentPreview)}`)
    console.log(`Framing preset preview: ${JSON.stringify({ ...framingPresetPreview, screenshot: framingPresetScreenshotPath })}`)
    console.log(`Main split framing preview: ${JSON.stringify(mainSplitTreatmentPreview)}`)
    console.log(`Corner framing export: ${JSON.stringify({ success: cornerExportResult.success, message: cornerExportResult.message, outputBytes: cornerOutputStats?.size ?? 0 })}`)
    console.log(`Main split framing export: ${JSON.stringify({ success: mainSplitExportResult.success, message: mainSplitExportResult.message, outputBytes: mainSplitOutputStats?.size ?? 0 })}`)
    console.log(`Framing transition export: ${JSON.stringify({ success: framingTransitionExportResult.success, message: framingTransitionExportResult.message, outputBytes: framingTransitionOutputStats?.size ?? 0 })}`)
