...
export function VideoSurface(): React.ReactElement {
  ...
  return <video ref={app.videoRef} className="video-surface" style={state.videoWidth > 0 && state.videoHeight > 0 ? { aspectRatio: `${state.videoWidth} / ${state.videoHeight}` } : undefined} src={state.currentFile.url} preload="metadata" onClick={app.handleVideoClick} onDoubleClick={app.handleVideoDoubleClick} onPlay={() => app.setState((current) => ({ ...current, isPlaying: true }))} onPlaying={app.clearPlaybackError} onCanPlay={app.clearPlaybackError} onPause={(event) => { app.setState((current) => ({ ...current, isPlaying: false })); app.persistPlaybackProgress(event.currentTarget.currentTime, true) }} onEnded={() => { app.playbackEndedRef.current = true; app.setState((current) => ({ ...current, isPlaying: false })); app.persistPlaybackProgress(0, true) }} onLoadedMetadata={onLoadedMetadata} onTimeUpdate={(event) => { const currentTime = event.currentTarget.currentTime; app.setState((current) => ({ ...current, currentTime, error: null })); app.persistPlaybackProgress(currentTime) }} onVolumeChange={(event) => { app.setState((current) => ({ ...current, volume: event.currentTarget.volume, muted: event.currentTarget.muted })) }} onError={app.handleMediaError} controls={false} />
}
