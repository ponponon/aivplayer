...
1. LanceDB 已写入 Flathub 源码构建模块，但尚未由 Flathub builder 验证；`onnxruntime-node`、Sharp 的 libvips 平台包以及 sherpa-onnx 的平台包仍是原生依赖阻塞项。可用 `npm run flatpak:audit-native -- --strict` 作为依赖审计门禁。
