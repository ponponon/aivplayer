...
import { getEditingFramingKeyframes, resolveEditingFramingAtTime } from '../../../core/editing/framing-operations'
...
export function VideoSurface(): React.ReactElement {
  ...
  const editingFramingKeyframes = app.isEditingMode && app.editingProject
    ? getEditingFramingKeyframes(getVideoClipSpans(app.editingProject.videoClips).map((span) => ({
      editedStartSeconds: span.editedStartSeconds,
      editedEndSeconds: span.editedEndSeconds,
      clip: currentEditingClip?.id === span.clip.id ? currentEditingClip : span.clip
    })))
    : []
  ...
  const framingResolution = !activeSplitPosition ? resolveEditingFramingAtTime(editingFramingKeyframes, app.editingCurrentTime) : null
  const isPunchIn = Boolean(framingResolution?.state.treatment === 'punch-in')
  const compactFramingTreatment = framingResolution?.state.treatment === 'corner-br' || framingResolution?.state.treatment === 'corner-tl' || framingResolution?.state.treatment === 'split-left' || framingResolution?.state.treatment === 'split-right' ? framingResolution.state.treatment : null
  const punchInOrigin = framingResolution?.state.anchor ?? 'center'
  const transformOrigin = framingResolution?.isTransitioning || compactFramingTreatment ? '50% 50%' : punchInOrigin === 'left' ? '0% 50%' : punchInOrigin === 'right' ? '100% 50%' : '50% 50%'
  ...
  const framingTransform = framingResolution?.isTransitioning
    ? `translate(${framingResolution.translateXPercent}%, ${framingResolution.translateYPercent}%) scale(${framingResolution.scale})`
    : isPunchIn
      ? `scale(${framingResolution?.scale ?? 1})`
      : compactFramingTreatment
        ? `translate(${framingResolution?.translateXPercent ?? 0}%, ${framingResolution?.translateYPercent ?? 0}%) scale(${framingResolution?.scale ?? 1})`
      : ''
  ...
  const videoTransform = [framingTransform, transitionTransform, clipMotionTransform].filter(Boolean).join(' ')
  ...
  const isCurrentVideo = (video: HTMLVideoElement): boolean => video === app.videoRef.current
  const syncCurrentVideoState = (video: HTMLVideoElement): boolean => {
    if (!isCurrentVideo(video)) return false
    app.syncPlaybackState(video)
    return true
  }
  ...
  return <><video key={mediaUrl || 'empty-media'} ref={app.videoRef} className={`video-surface ${isPunchIn ? 'is-punch-in' : ''} ${compactFramingTreatment ? `is-framing-${compactFramingTreatment}` : ''} ${activeSplitPosition ? `is-${activeSplitPosition}` : ''} ${personMattePreviewFrameUrl ? 'is-person-matte' : ''}`} data-framing-treatment={framingResolution?.state.treatment ?? 'full'} data-testid={personMattePreviewFrameUrl ? 'editing-person-matte-preview' : undefined} style={videoStyle} src={mediaUrl} preload="metadata" onClick={app.handleVideoClick} onDoubleClick={app.handleVideoDoubleClick} onPlay={(event) => { syncCurrentVideoState(event.currentTarget) }} onPlaying={(event) => { if (!syncCurrentVideoState(event.currentTarget)) return; app.clearPlaybackError() }} onCanPlay={(event) => { if (!syncCurrentVideoState(event.currentTarget)) return; app.clearPlaybackError() }} onPause={(event) => { const video = event.currentTarget; if (!syncCurrentVideoState(video)) return; const currentTime = video.currentTime; if (!app.isEditingMode) app.persistPlaybackProgress(currentTime, true) }} onEnded={(event) => { const video = event.currentTarget; if (!isCurrentVideo(video)) return; app.editingResumePlaybackRef.current = false; if (app.isEditingMode) { app.playbackEndedRef.current = true; app.syncPlaybackState(video); return }; app.handlePlaybackEnded(video) }} onLoadedMetadata={onLoadedMetadata} onTimeUpdate={(event) => { const video = event.currentTarget; if (!syncCurrentVideoState(video)) return; const currentTime = video.currentTime; app.setState((current) => ({ ...current, currentTime, error: null })); if (!app.isEditingMode) app.persistPlaybackProgress(currentTime) }} onVolumeChange={(event) => { const video = event.currentTarget; if (!isCurrentVideo(video)) return; const { volume, muted } = video; app.setState((current) => ({ ...current, volume, muted })) }} onError={(event) => { const video = event.currentTarget; if (!syncCurrentVideoState(video)) return; app.handleMediaError(event) }} controls={false} />{personMatteTrackProgress?.status === 'processing' ? <div className="editing-person-matte-track-progress" data-testid="editing-person-matte-track-progress" role="status" aria-live="polite"><span>{copy.editing.personMatteProcessing(personMatteTrackProgress.processedFrames, personMatteTrackProgress.totalFrames)}</span><strong>{progressPercent}%</strong><i><b style={{ width: `${progressPercent}%` }} /></i></div> : null}</>
}
