...
describe('waveform integration', () => {
  ...
  it('keeps the waveform track rendered and linked to editor seeking', () => {
    ...
    expect(track).toContain('onPointerMove')
    expect(track).toContain('setPointerCapture(event.pointerId)')
    expect(track).toContain('is-dragging')
    ...
    expect(readSource('scripts/smoke-editing-playhead.ts')).toContain('editing-waveform-track')
